  ...
  return $.getJSON(
    ...
    function (data) {
      var cObj = {
        ...
        type:
          data.alertDetail.messageType == "Update"
            ? " Update"
            : data.alertDetail.messageType == "Cancel"
              ? " Cancellation"
              : "",
...
async function getCrawlAlerts() {
  ...
  var url =
    ...
    locationDataHeaders.mainData.crawlAlerts +
    "&format=json&language=en-US&apiKey=" +
    api_key;
  ...
  try {
    ...
    weatherData.crawlAlerts = {
      locationName:
        systemSettings.mainCity.bulletinName == ""
          ? systemSettings.mainCity.locationName + " Area"
          : systemSettings.mainCity.bulletinName,
      warnings: [],
    };
    ...
    if (data && data.alerts) {
      ...
      console.log(
        "All crawl alerts fetched and sorted:",
        weatherData.crawlAlerts.warnings,
      );
...
function getCoreData() {
  async function getBulletinAlerts() {
    var url =
      "https://api.weather.com/v3/alerts/headlines?" +
      locationDataHeaders.mainData.alerts.mainLoc +
      "&format=json&language=en-US&apiKey=" +
      api_key;
    ...
    try {
      ...
      if (data && data.alerts) {
        ...
        for (var i = 0; i < data.alerts.length; i++) {
          ...
          if (eventDesc == "Ozone Action Day") {
            weatherData.airQuality.ozoneAction = true;
          }
          if (
            eventDesc == "Freeze Warning" ||
            eventDesc == "Hard Freeze Warning" ||
            eventDesc == "Frost Advisory"
          ) {
            weatherData.frostFreezeWarning = true;
          }

          function addAlert() {
            var alertEvent =
              weatherData.crawlAlerts.warnings.length > 0
                ? weatherData.crawlAlerts.warnings[0].event
                : "none";
            if (
              !warningSettings[eventDesc].marine &&
              eventDesc != "Special Weather Statement"
            ) {
              if (
                (warningSettings[eventDesc].severe &&
                  eventDesc != alertEvent) ||
                !warningSettings[eventDesc].severe
              ) {
                weatherData.alerts.mainLoc.warnings.push(
                  data.alerts[i].headlineText,
                );
              }
            }
          }
        ...
        weatherData.alerts.mainLoc.warnings.sort(
        ...
        weatherData.alerts.mainLoc.pages = Math.ceil(
          $(".gbul .alerts").height() / 534,
        );
        ...
        console.log(
          "All bulletin alerts fetched and sorted:",
          weatherData.alerts.mainLoc.warnings,
        );
      } else {
        weatherData.alerts.mainLoc = {
          locationname: systemSettings.mainCity.locationName,
  ...
  function getCurrent() {
    var url =
      ...
      locationDataHeaders.mainData.currentConditions.english.mainLoc +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      weatherData.currentConditions.english.mainLoc.locationName =
        systemSettings.mainCity.obsName;
      weatherData.currentConditions.english.mainLoc.noReport = false;
      weatherData.currentConditions.english.mainLoc.humidity =
        data.relativeHumidity + "%";
      weatherData.currentConditions.english.mainLoc.dewPoint =
        data.temperatureDewPoint;
      weatherData.currentConditions.english.mainLoc.pressure =
        data.pressureAltimeter.toFixed(2);
      weatherData.currentConditions.english.mainLoc.pressureTrend =
        data.pressureTendencyCode;
      weatherData.currentConditions.english.mainLoc.wind =
        data.windDirectionCardinal == "CALM" || data.windSpeed == 0
          ? "Calm"
          : data.windDirectionCardinal + " " + data.windSpeed;
      weatherData.currentConditions.english.mainLoc.gusts =
        data.windGust != null ? data.windGust + " mph" : "None";
      weatherData.currentConditions.english.mainLoc.feelsLikeTitle =
        data.temperature > 65 ? "Heat Index" : "Wind Chill";
      weatherData.currentConditions.english.mainLoc.feelsLike =
        data.temperatureFeelsLike == data.temperature
          ? "none"
          : data.temperatureFeelsLike;
      weatherData.currentConditions.english.mainLoc.icon = data.iconCodeExtend;
      weatherData.currentConditions.english.mainLoc.condition = getCond(
        data.iconCodeExtend,
        "current",
      );
      weatherData.currentConditions.english.mainLoc.temperature =
        data.temperature;
      weatherData.almanac.sunrise = dateFns
        .format(new Date(data.sunriseTimeLocal), "h:mm a")
        .toLowerCase();
      weatherData.almanac.sunset = dateFns
        .format(new Date(data.sunsetTimeLocal), "h:mm a")
        .toLowerCase();
    }).fail(function () {
      weatherData.currentConditions.english.mainLoc.locationName =
        systemSettings.mainCity.obsName;
  ...
  function getNearby(num) {
    var url =
      "https://api.weather.com/v3/wx/observations/current?" +
      locationDataHeaders.mainData.nearbyCities[num] +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      weatherData.nearbyCities.cities[num].noReport = false;
      weatherData.nearbyCities.cities[num].locationName =
        systemSettings.nearbyCities.cities[num].obsName;
  ...
  function getLF() {
    var url =
      ...
      locationDataHeaders.mainData.localForecast.mainLoc +
    ...
    $.getJSON(url, function (data) {
      ...
      weatherData.localForecast.mainLoc.locationName =
        systemSettings.mainCity.locationName;
      weatherData.localForecast.mainLoc.noReport = false;
      for (
        var i = 0;
        i < weatherData.localForecast.mainLoc.times.length;
        i++, ii++
      ) {
        ...
        weatherData.localForecast.mainLoc.times[i].timetitle =
          data.daypart[0].daypartName[ii]
            .replaceAll(" night", " Night")
            .replaceAll("Tomorrow", data.dayOfWeek[ni]);
        weatherData.localForecast.mainLoc.times[i].forecast =
          data.daypart[0].narrative[ii].replace("F. ", ". ");
      }
    }).fail(function () {
      weatherData.localForecast.mainLoc.locationName =
        systemSettings.mainCity.locationName;
  ...
  function getDayPart() {
    var url =
      ...
      locationDataHeaders.mainData.dayPart.english.mainLoc +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    ...
    $.getJSON(url, function (data) {
      ...
      switch (true) {
        ...
        case currenthr < 9:
          weatherData.dayPart.english.mainLoc.header = "Today's Forecast";
          targetHours = [12, 15, 17, 20];
          break;
        case currenthr < 12:
          weatherData.dayPart.english.mainLoc.header = "Tonight's Forecast";
          targetHours = [15, 17, 20, 0];
          break;
        case currenthr < 14:
          weatherData.dayPart.english.mainLoc.header = "Tonight's Forecast";
          targetHours = [17, 20, 0, 6];
          break;
        case currenthr < 17:
      ...
      var ii = 0;
      weatherData.dayPart.english.mainLoc.locationName =
        systemSettings.mainCity.locationName;
      ...
      for (var i = 0; i < data.validTimeLocal.length; i++) {
        if (ii < 4) {
          ...
          if (dpTime == targetHours[ii]) {
            weatherData.dayPart.english.mainLoc.times[ii].time =
              timeTitleCreate(data.validTimeLocal[i]);
            weatherData.dayPart.english.mainLoc.times[ii].icon =
              data.iconCodeExtend[i];
            weatherData.dayPart.english.mainLoc.times[ii].condition = getCond(
              data.iconCodeExtend[i],
              "daypart",
            );
            weatherData.dayPart.english.mainLoc.times[ii].temperature =
              data.temperature[i];
            weatherData.dayPart.english.mainLoc.times[ii].wind =
              data.windDirectionCardinal[i] == "CALM" || data.windSpeed[i] == 0
                ? "Calm"
                : data.windDirectionCardinal[i] + " " + data.windSpeed[i];
            ii++;
          }
        } else {
          break;
  ...
  function getExtended() {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.extendedForecast.english.mainLoc +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      var today = longWeekDays[new Date().getDay()];
      weatherData.extendedForecast.english.mainLoc.locationName =
        systemSettings.mainCity.locationName;
      ...
      for (
        var i = 0;
        i < weatherData.extendedForecast.english.mainLoc.days.length;
        i++, ii++, dpi = dpi + 2
      ) {
        weatherData.extendedForecast.english.mainLoc.days[i].day =
          data.dayOfWeek[ii].substring(0, 3);
        weatherData.extendedForecast.english.mainLoc.days[i].icon =
          data.daypart[0].iconCodeExtend[dpi];
        weatherData.extendedForecast.english.mainLoc.days[i].condition =
          getCond(
            weatherData.extendedForecast.english.mainLoc.days[i].icon,
            "forecast",
          );
        ...
        weatherData.extendedForecast.english.mainLoc.days[i].high =
  ...
  function getAlmanac() {
    var url =
      "https://api.weather.com/v3/wx/almanac/daily/5day?" +
      locationDataHeaders.mainData.almanac +
      `&format=json&units=e&startDay=${dateFns.format(new Date(), "d")}&startMonth=${dateFns.format(new Date(), "M")}&apiKey=` +
      api_key;
    $.getJSON(url, function (data) {
      weatherData.almanac.locationName =
        systemSettings.mainCity.almanacLocationName;
      ...
      function getMoons() {
        var ii = 0;
        $.getJSON(
          `https://www.icalendar37.net/lunar/api/?lang=en&month=${dateFns.format(new Date(), "M")}&year=${dateFns.format(new Date(), "yyyy")}`,
          function (data) {
            ...
            if (weatherData.almanac.moonphases[3].date == "") {
              setTimeout(() => {
                $.getJSON(
                  `https://www.icalendar37.net/lunar/api/?lang=en&month=${dateFns.format(dateFns.addMonths(new Date(), 1), "M")}&year=${dateFns.format(dateFns.addMonths(new Date(), 1), "yyyy")}`,
                  function (data) {
                    for (phase in data.phase) {
                      if (data.phase[phase].isPhaseLimit != false) {
                        weatherData.almanac.moonphases[ii].moon = {
...
function getExtraCore(locNum) {
  async function getBulletinAlerts() {
    var url =
      "https://api.weather.com/v3/alerts/headlines?" +
      locationDataHeaders.mainData.alerts.extraLoc[locNum] +
      "&format=json&language=en-US&apiKey=" +
      api_key;
    weatherData.airQuality.ozoneAction = false;
    weatherData.frostFreezeWarning = false;
    try {
      ...
      weatherData.alerts.extraLoc[locNum] = {
        locationName:
          systemSettings.extraCity.cities[locNum].bulletinName == ""
            ? systemSettings.extraCity.cities[locNum].locationName + " Area"
            : systemSettings.extraCity.cities[locNum].bulletinName,
        warnings: [],
      };
      ...
      if (data && data.alerts) {
        ...
        for (var i = 0; i < data.alerts.length; i++) {
          ...
          function addAlert() {
            if (
              ...
              if (
                (warningSettings[eventDesc].severe &&
                  eventDesc != weatherData.crawlAlerts.warnings[0].event) ||
                !warningSettings[eventDesc].severe
              ) {
                weatherData.alerts.extraLoc[locNum].warnings.push(
                  data.alerts[i].headlineText,
                );
              }
            }
        ...
        weatherData.alerts.extraLoc[locNum].warnings.sort(
          (a, b) => a.sortNum - b.sortNum,
        );

        console.log(
          "All extra bulletin alerts fetched and sorted:",
          weatherData.alerts.extraLoc[locNum].warnings,
        );
      } else {
        weatherData.alerts.extraLoc[locNum] = {
          locationname: systemSettings.extraCity.cities[locNum].locationName,
          warnings: [],
        };
      ...
      $.getJSON(url, function (data) {
        ...
        try {
          if (data != undefined) {
            for (var i = 0; i < data.alerts.length; i++) {
              if (
                !warningSettings[data.alerts[i].eventDescription].marine &&
                data.alerts[i].eventDescription != "Special Weather Statement"
              ) {
                weatherData.alerts.extraLoc[locNum].warnings.push(
                  data.alerts[i].headlineText,
                );
            ...
            for (
              var i = 0;
              i < weatherData.alerts.extraLoc[locNum].warnings.length;
              i++
            ) {
              atext =
                atext +
                weatherData.alerts.extraLoc[locNum].warnings[i] +
                " " +
                "\n\n";
            }
            $(".gbul .alerts").text(atext.slice(0, -2));
            weatherData.alerts.extraLoc[locNum].pages = Math.ceil(
              $(".gbul .alerts").height() / 534,
            );
            ...
            weatherData.alerts.extraLoc[locNum] = {
              locationname:
                systemSettings.extraCity.cities[locNum].locationName,
              warnings: [],
            };
            weatherData.alerts.extraLoc[locNum].alertsAmount = 0;
          }
  ...
  function getCurrent() {
    ...
    var url =
      "https://api.weather.com/v3/wx/observations/current?" +
      locationDataHeaders.mainData.currentConditions.english.extraLoc[locNum] +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    var eLocData = {
      noReport: true,
  ...
  function getLF() {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.localForecast.extraLoc[locNum] +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    //console.log(url)
    var eLocLF = {
    ...
    $.getJSON(url, function (data) {
      ...
      for (var i = 0; i < eLocLF.times.length; i++, ii++) {
        ...
        eLocLF.times[i].timetitle = data.daypart[0].daypartName[ii]
          .replaceAll(" night", " Night")
          .replaceAll("Tomorrow", data.dayOfWeek[ni]);
        eLocLF.times[i].forecast = data.daypart[0].narrative[ii].replace(
          "F. ",
          ". ",
        );
      }
      ...
      weatherData.localForecast.extraLoc[locNum].locationName =
        systemSettings.mainCity.locationName;
  ...
  function getDayPart() {
    ...
    var url =
      "https://api.weather.com/v3/wx/forecast/hourly/2day?" +
      locationDataHeaders.mainData.dayPart.english.extraLoc[locNum] +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var elocData = {
      noReport: true,
      ...
      for (var i = 0; i < data.validTimeLocal.length; i++) {
        if (ii < 4) {
          ...
          if (dpTime == targetHours[ii]) {
            ...
            elocData.times[ii].condition = getCond(
              data.iconCodeExtend[i],
              "daypart",
            );
            elocData.times[ii].temperature = data.temperature[i];
            elocData.times[ii].wind =
              data.windDirectionCardinal[i] == "CALM" || data.windSpeed[i] == 0
                ? "Calm"
                : data.windDirectionCardinal[i] + " " + data.windSpeed[i];
            ii++;
  ...
  function getExtended() {
    ...
    var url =
      ...
      systemSettings.extraCity.cities[locNum].lat +
      "," +
      systemSettings.extraCity.cities[locNum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var eLocData = {
      noReport: true,
...
function getSpanishData() {
  function getCurrent() {
    var url =
      "https://api.weather.com/v3/wx/observations/current?icaoCode=" +
      systemSettings.mainCity.obsIcaoCode +
      "&units=e&language=es-US&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      weatherData.currentConditions.spanish.locationName =
        systemSettings.mainCity.obsName;
      weatherData.currentConditions.spanish.noReport = false;
      weatherData.currentConditions.spanish.humidity =
        data.relativeHumidity + "%";
      weatherData.currentConditions.spanish.dewPoint = data.temperatureDewPoint;
      weatherData.currentConditions.spanish.pressure =
        data.pressureAltimeter.toFixed(2);
      weatherData.currentConditions.spanish.pressureTrend =
        data.pressureTendencyCode;
      weatherData.currentConditions.spanish.wind =
        data.windDirectionCardinal == "CALM" || data.windSpeed == 0
          ? "Calm"
          : data.windDirectionCardinal + " " + data.windSpeed;
      weatherData.currentConditions.spanish.gusts =
        data.windGust != null ? data.windGust + " mph" : "None";
      weatherData.currentConditions.spanish.feelsLikeTitle =
        "Sensación téermica";
      weatherData.currentConditions.spanish.feelsLike =
        data.temperatureFeelsLike == data.temperature
          ? "none"
          : data.temperatureFeelsLike;
      weatherData.currentConditions.spanish.icon = data.iconCodeExtend;
      weatherData.currentConditions.spanish.condition = getCond(
        data.iconCodeExtend,
        "current",
        "spanish",
      );
  ...
  function getDayPart() {
    var url =
      "https://api.weather.com/v3/wx/forecast/hourly/2day?geocode=" +
      systemSettings.mainCity.lat +
      "," +
      systemSettings.mainCity.lon +
    ...
    $.getJSON(url, function (data) {
      ...
      var ii = 0;
      weatherData.dayPart.spanish.locationName =
        systemSettings.mainCity.locationName;
      ...
      for (var i = 0; i < data.validTimeLocal.length; i++) {
        if (ii < 4) {
          var dpTime = dateFns.getHours(data.validTimeLocal[i]);
          if (dpTime == targetHours[ii]) {
            weatherData.dayPart.spanish.times[ii].time = timeTitleCreate(
              data.validTimeLocal[i],
            );
            weatherData.dayPart.spanish.times[ii].icon = data.iconCodeExtend[i];
            weatherData.dayPart.spanish.times[ii].condition = getCond(
              data.iconCodeExtend[i],
              "daypart",
              "spanish",
            );
            weatherData.dayPart.spanish.times[ii].temperature =
              data.temperature[i];
            weatherData.dayPart.spanish.times[ii].wind =
              data.windDirectionCardinal[i] == "CALMA" || data.windSpeed[i] == 0
                ? "Calma"
                : data.windDirectionCardinal[i] + " " + data.windSpeed[i];
            ii++;
          }
  ...
  function getExtended() {
    ...
    $.getJSON(url, function (data) {
      var today = longWeekDays[new Date().getDay()];
      weatherData.extendedForecast.spanish.locationName =
        systemSettings.mainCity.locationName;
      ...
      for (
        var i = 0;
        i < weatherData.extendedForecast.spanish.days.length;
        i++, ii++, dpi = dpi + 2
      ) {
        weatherData.extendedForecast.spanish.days[i].day = data.dayOfWeek[ii]
          .substring(0, 3)
          .replaceAll("Vie", "Vier")
          .replaceAll("Mié", "Miér");
        weatherData.extendedForecast.spanish.days[i].icon =
          data.daypart[0].iconCodeExtend[dpi];
        weatherData.extendedForecast.spanish.days[i].condition = getCond(
          weatherData.extendedForecast.spanish.days[i].icon,
          "forecast",
          "spanish",
        );
        ...
        weatherData.extendedForecast.spanish.days[i].high =
          data.temperatureMax[ii];
      }
    }).fail(function () {
      weatherData.extendedForecast.spanish.locationName =
...
function getTrafficData() {
  function getTrafficReport() {
    var trafurl =
      "https://data.traffic.hereapi.com/v7/incidents?in=circle:" +
      systemSettings.traffic.lat +
      "," +
      systemSettings.traffic.lon +
      ";r=4000&locationReferencing=tmc&lang=en-US&type=construction,accident&apiKey=" +
      traf_key;
    console.log(trafurl);
    $.getJSON(trafurl, function (data) {
      ...
      weatherData.trafficReport.locationName =
        systemSettings.traffic.locationName;
      var t = 0;
      if (data.results.length > 0) {
        for (var i = 0; i < data.results.length; i++) {
          if (t < 6) {
            ...
            trafficObj.title =
              incidentType(data.results[i].incidentDetails.type) +
              parseStreetName(
                data.results[i].incidentDetails.description.value,
              );
            trafficObj.impact = incidentImpact(
              data.results[i].incidentDetails.criticality,
            );
            trafficObj.desc =
              spacer[
                incidentImpact(data.results[i].incidentDetails.criticality)
              ] + data.results[i].incidentDetails.description.value;
            if (
              parseStreetName(
                data.results[i].incidentDetails.description.value,
  ...
  function getTrafficFlow(routenum) {
    var url =
      ...
      systemSettings.traffic.routes[routenum].from +
      "&destination=" +
      systemSettings.traffic.routes[routenum].to +
      "&return=summary,travelSummary,polyline&apiKey=" +
      traf_key;
    console.log(url);
    $.getJSON(url, function (data) {
      weatherData.trafficFlow.locationName =
        systemSettings.traffic.locationName;
      weatherData.trafficFlow.noReport = false;
      var flowObj = {
        routeIcon: "",
      ...
      flowObj.baseSpeed = Math.round(
        metStoMPH(
          data.routes[0].sections[0].summary.length /
            data.routes[0].sections[0].summary.baseDuration,
        ),
      );
      flowObj.mph = Math.round(
        metStoMPH(
          data.routes[0].sections[0].summary.length /
      ...
      flowObj.speed =
        flowObj.mph >= flowObj.baseSpeed
          ? "CLEAR"
          : metStoMPH(
              data.routes[0].sections[0].summary.length /
                data.routes[0].sections[0].summary.duration,
            );
      flowObj.travelTime = Math.round(
      ...
      flowObj.routeIcon =
        "/images/roadSigns/" +
        systemSettings.traffic.routes[routenum].routeIcon +
        ".png";
      setTimeout(() => {
...
function getTravelData() {
  function getTravelMapData(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.travel.regionalMap.cities[citynum].lat +
      "," +
      systemSettings.travel.regionalMap.cities[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var tfObj = {
      tfData: [
    ...
    $.getJSON(url, function (data) {
      ...
      var hourOffset = getHourAtOffset(
        Number.parseInt(data.validTimeLocal[0].slice(-5, -2)),
      ); //(new Date(data.validTimeLocal[0]).getTimezoneOffset()/60)*100
      ...
      for (var i = 0; i < tfObj.tfData.length; i++, ii++, dpi = dpi + 2) {
        ...
        weatherData.travelForecast.dayName[i] =
          weatherData.travelForecast.dayName[i] == "null"
            ? data.dayOfWeek[ii]
            : weatherData.travelForecast.dayName[i];
        tfObj.tfData[i].locationName =
          systemSettings.travel.regionalMap.cities[citynum].locationName;
  ...
  function getDestinationData() {
    var url =
      "https://api.weather.com/v3/aggcommon/v3-wx-forecast-daily-5day?geocodes=";
    for (let i = 0; i < systemSettings.travel.destinationForecast.length; i++) {
      url =
        url +
        `${systemSettings.travel.destinationForecast[i].lat},${systemSettings.travel.destinationForecast[i].lon};`;
    }
    ...
    $.getJSON(url, function (data) {
      data.forEach((ajaxedLoc, i) => {
        ...
        if (
          data[0]["v3-wx-forecast-daily-5day"].daypart[0].daypartName[0] == null
        ) {
          hourOffset = true;
        }
        ...
        try {
          ...
          dfObj.locationName =
            systemSettings.travel.destinationForecast[i].locationName;
          for (var ji = 0; ji < dfObj.days.length; ji++, ii++, dpi = dpi + 2) {
            weatherData.destinationForecast.dayName[ji] =
              weatherData.destinationForecast.dayName[ji] == "null"
                ? ajaxedLoc["v3-wx-forecast-daily-5day"].dayOfWeek[
                    ii
                  ].substring(0, 3)
                : weatherData.destinationForecast.dayName[ji];
            if (
              weatherData.destinationForecast.dayName[ji] !=
              ajaxedLoc["v3-wx-forecast-daily-5day"].dayOfWeek[ii].substring(
                0,
                3,
              )
            ) {
            ...
            dfObj.days[ji].high =
              ajaxedLoc["v3-wx-forecast-daily-5day"].temperatureMax[ii];
            if (
              ajaxedLoc["v3-wx-forecast-daily-5day"].dayOfWeek[ii] ==
              today /* && new Date().getHours() >= 4*/
            ) {
              dfObj.days[ji].low = "";
            } else {
              dfObj.days[ji].low =
                ajaxedLoc["v3-wx-forecast-daily-5day"].temperatureMin[ii];
            }
            dfObj.days[ji].icon =
              ajaxedLoc["v3-wx-forecast-daily-5day"].daypart[0].iconCodeExtend[
                dpi
              ];
          }
        } catch (error) {
...
function getHealthData() {
  function getOdActivityData() {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.outdoorActivity +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      var today = longWeekDays[new Date().getDay()];
      ...
      weatherData.outdoorActivity.locationName =
        systemSettings.health.locationName;
      ...
      weatherData.outdoorActivity.low =
        data.dayOfWeek[ii] == today ? "none" : data.temperatureMin[ii];
      weatherData.outdoorActivity.feelsLike =
        data.daypart[0].temperature[dpi] > 65
          ? "Heat Index   " + data.daypart[0].temperatureHeatIndex[dpi]
          : "Wind Chill   " + data.daypart[0].temperatureWindChill[dpi];
      weatherData.outdoorActivity.feelsLikeNum =
        data.daypart[0].temperature[dpi] > 65
          ? data.daypart[0].temperatureHeatIndex[dpi]
          : data.daypart[0].temperatureWindChill[dpi];
      weatherData.outdoorActivity.precipChance =
        data.daypart[0].precipChance[dpi] + "%";
      weatherData.outdoorActivity.humidity =
        data.daypart[0].relativeHumidity[dpi] + "%";
      weatherData.outdoorActivity.wind =
        data.daypart[0].windDirectionCardinal[dpi] == "CALM" ||
        data.daypart[0].windSpeed[dpi] == 0
          ? "Calm"
          : data.daypart[0].windDirectionCardinal[dpi] +
            " " +
  ...
  function getPollenData() {
    var url =
      ...
      locationDataHeaders.mainData.pollen +
      "/observations/pollen.json?language=en-US&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      console.log(data);
      if (data.pollenobservations !== undefined) {
        if (
          data.pollenobservations[0].stn_cmnt_cd != "A" &&
          data.pollenobservations[0].stn_cmnt_cd != "B" &&
          data.pollenobservations[0].stn_cmnt_cd != "C" &&
          data.pollenobservations[0].stn_cmnt_cd != "D"
        ) {
          weatherData.pollen.noReport = false;
          weatherData.pollen.locationName =
            systemSettings.health.locationName == ""
              ? data.pollenobservations[0].loc_nm
              : systemSettings.health.locationName;
          weatherData.pollen.date = dateFns.format(
            new Date(data.pollenobservations[0].rpt_dt),
            "MMM d",
          );
          weatherData.pollen.pollenCount =
            data.pollenobservations[0].total_pollen_cnt == null
              ? ""
              : data.pollenobservations[0].total_pollen_cnt;
          weatherData.pollen.pollenCountCategory =
            data.pollenobservations[0].total_pollen_desc;
          weatherData.pollen.indices[0] =
            data.pollenobservations[0].pollenobservation[0].pollen_idx == 9
              ? 0
              : data.pollenobservations[0].pollenobservation[0].pollen_idx;
          weatherData.pollen.indices[1] =
            data.pollenobservations[0].pollenobservation[1].pollen_idx == 9
              ? 0
              : data.pollenobservations[0].pollenobservation[1].pollen_idx;
          weatherData.pollen.indices[2] =
            data.pollenobservations[0].pollenobservation[2].pollen_idx == 9
              ? 0
              : data.pollenobservations[0].pollenobservation[2].pollen_idx;
          weatherData.pollen.indices[3] =
            data.pollenobservations[0].pollenobservation[3].pollen_idx == 9
              ? 0
              : data.pollenobservations[0].pollenobservation[3].pollen_idx;
          weatherData.pollen.treeType =
            data.pollenobservations[0].treenames[0].tree_nm != "No Report"
              ? toSentenceCase(data.pollenobservations[0].treenames[0].tree_nm)
              : "";
        } else {
  ...
  function getAchesPains() {
    var achesurl =
      "https://api.weather.com/v2/indices/achePain/daypart/3day?" +
      locationDataHeaders.mainData.achesBreathe +
    ...
    $.getJSON(achesurl, function (achesdata) {
      ...
      weatherData.achesBreathe.locationName =
        systemSettings.health.locationName;
      weatherData.achesBreathe.dayName = dateFns.format(
        new Date(achesdata.achesPainsIndex12hour.fcstValidLocal[ii]),
        "eeee",
      );
      weatherData.achesBreathe.achesIndex =
        achesdata.achesPainsIndex12hour.achesPainsIndex[ii];
      weatherData.achesBreathe.achesWord =
        achesdata.achesPainsIndex12hour.achesPainsCategory[ii];
    }).fail(function () {
    ...
    var breathurl =
      "https://api.weather.com/v2/indices/breathing/daypart/3day?" +
      locationDataHeaders.mainData.achesBreathe +
      "&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(breathurl, function (breathdata) {
      ...
      weatherData.achesBreathe.breatheIndex =
        breathdata.breathingIndex12hour.breathingIndex[ii];
      weatherData.achesBreathe.breatheWord =
        breathdata.breathingIndex12hour.breathingCategory[ii];
      weatherData.achesBreathe.dayName = dateFns.format(
        new Date(breathdata.breathingIndex12hour.fcstValidLocal[ii]),
        "eeee",
      );
    }).fail(function () {
  ...
  function getAirQuality() {
    var url =
      ...
      locationDataHeaders.mainData.airQuality +
      "&language=en-US&scale=EPA&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      weatherData.airQuality.noReport = false;
      weatherData.airQuality.locationName = systemSettings.health.locationName;
      weatherData.airQuality.dayName = dateFns.format(
        new Date(data.globalairquality.expireTimeGmt * 1000),
        "eeee",
      );
      weatherData.airQuality.index = Math.round(
        data.globalairquality.airQualityCategoryIndex / 2,
      );
      weatherData.airQuality.primaryPollutant =
        data.globalairquality.primaryPollutant
          .replace("CO", "Ozone")
          .replace("NO2", "Ozone")
          .replace("O3", "Ozone")
          .replace("PM10", "Fine\nParticulates")
  ...
  function getUVData() {
    var currenturl =
      "https://api.weather.com/v3/wx/observations/current?" +
      locationDataHeaders.mainData.uvIndexCurrent +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(currenturl, function (currentdata) {
      if (currentdata.uvIndex >= 0) {
        ...
        weatherData.uvIndex.current.index =
          currentdata.uvIndex > 10 ? 10 : currentdata.uvIndex;
    ...
    var forecasturl =
      "https://api.weather.com/v2/indices/uv/hourly/48hour?" +
      locationDataHeaders.mainData.uvIndexFcst +
      "&language=en-US&format=json&apiKey=" +
      api_key;
    var currenthr = dateFns.getHours(new Date());
    $.getJSON(forecasturl, function (forecastdata) {
      ...
      for (
        var i = 0;
        i < forecastdata.uvIndex1hour.fcstValidLocal.length;
        i++
      ) {
        if (ii < 3) {
          var uvTime = dateFns.getHours(
            forecastdata.uvIndex1hour.fcstValidLocal[i],
          );
          if (uvTime == targetHours[ii]) {
            if (forecastdata.uvIndex1hour.uvIndex[i] >= 0) {
              weatherData.uvIndex.forecast.times[ii].hour = timeTitleCreate(
                forecastdata.uvIndex1hour.fcstValidLocal[i],
              );
              weatherData.uvIndex.forecast.times[ii].day =
                forecastdata.uvIndex1hour.dow[i].substring(0, 3);
              weatherData.uvIndex.forecast.times[ii].index =
                forecastdata.uvIndex1hour.uvIndex[i] > 10
                  ? 10
                  : forecastdata.uvIndex1hour.uvIndex[i];
              weatherData.uvIndex.forecast.times[ii].word =
                forecastdata.uvIndex1hour.uvDesc[i];
              ii++;
            } else {
...
function getAirporData() {
    ...
    $.getJSON("/airports", function (eventdata) {
      for (const airportevent of eventdata) {
        ...
        if (airportevent.groundStop != null) {
          if (delay.closed == false) {
            delay.departureDelay =
              differenceUTC(airportevent.groundStop.endTime) >
              delay.departureDelay
                ? differenceUTC(airportevent.groundStop.endTime)
                : delay.departureDelay;
            delay.departureDelayReason =
              delay.departureDelayReason == ""
                ? airportevent.groundStop.impactingCondition
                : delay.departureDelayReason +
                  ", " +
                  airportevent.groundStop.impactingCondition;
          }
        ...
        if (airportevent.groundDelay != null) {
          if (delay.closed == false) {
            delay.departureDelay =
              airportevent.groundDelay.avgDelay > delay.departureDelay
                ? airportevent.groundDelay.avgDelay
                : delay.departureDelay;
            delay.departureDelayReason =
              delay.departureDelayReason == ""
                ? airportevent.groundDelay.impactingCondition
                : delay.departureDelayReason +
                  ", " +
                  airportevent.groundDelay.impactingCondition;
          }
        ...
        if (airportevent.arrivalDelay != null) {
          delay.arrivalDelay =
            airportevent.arrivalDelay.averageDelay > delay.arrivalDelay
        ...
        if (airportevent.departureDelay != null) {
          if (delay.closed == false) {
            delay.departureDelay =
              airportevent.departureDelay.averageDelay > delay.departureDelay
      ...
      for (var i = 0; i < weatherData.airportDelayList.length; i++) {
        if (weatherData.airportDelayList[i].arrivalDelay != "Closed") {
          if (weatherData.airportDelayList[i].arrivalDelay != 0) {
            weatherData.airportDelayList[i].arrivalDelay =
              weatherData.airportDelayList[i].arrivalDelay;
            weatherData.airportDelayList[i].arrivalDelay = formatMinutes(
              weatherData.airportDelayList[i].arrivalDelay,
            );
            ...
            weatherData.airportDelayList[i].arrivalDelay =
              "<em>" +
              ogATime
                .replaceAll(" hr ", "</em> hr <em>")
                .replaceAll(" min", "</em> min")
                .replaceAll(" hrONLYH", "</em> hr");
          } else {
        ...
        if (weatherData.airportDelayList[i].departureDelay != "Closed") {
          if (weatherData.airportDelayList[i].departureDelay != 0) {
            weatherData.airportDelayList[i].departureDelay =
              weatherData.airportDelayList[i].departureDelay;
            weatherData.airportDelayList[i].departureDelay = formatMinutes(
              weatherData.airportDelayList[i].departureDelay,
            );
  ...
  function getLocalAirports(num) {
    var url =
      "https://api.weather.com/v3/wx/observations/current?iataCode=" +
      systemSettings.airport.main[num].iataCode +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      var airportObj = {
      ...
      var airportObj = {
        ...
        iataCode: "",
        airportName: "",
        arrivalDelay: "No Report",
        arrivalDelayReason: "",
        departureDelay: "No Report",
        departureDelayReason: "",
        icon: "",
        condition: "",
        temperature: "",
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < systemSettings.airport.main.length; i++) {
      if (
        systemSettings.airport.main.length ==
        weatherData.localAirportConditions.length
      ) {
        break;
      }
      setTimeout(() => {
  ...
  function getNatAirports(num) {
    var url =
      "https://api.weather.com/v3/wx/observations/current?iataCode=" +
      systemSettings.airport.national[num].iataCode +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(url, function (data) {
      var airportObj = {
...
function getIntlData() {
  function getIntlMapDataCA(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.canada[citynum].lat +
      "," +
      systemSettings.international.map.canada[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = {
      noReport: true,
    ...
    $.getJSON(url, function (data) {
      ...
      imObj.locationName =
        systemSettings.international.map.canada[citynum].locationName;
  ...
  function getIntlMapDataMX(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.mexico[citynum].lat +
      "," +
      systemSettings.international.map.mexico[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    ...
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.mexico[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataCB(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.caribbean[citynum].lat +
      "," +
      systemSettings.international.map.caribbean[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.caribbean[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataSA(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.southamerica[citynum].lat +
      "," +
      systemSettings.international.map.southamerica[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.southamerica[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  for (
    var i = 0;
    i < systemSettings.international.map.southamerica.length;
    i++
  ) {
    getIntlMapDataSA(i);
  }
  function getIntlMapDataGB(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.britain[citynum].lat +
      "," +
      systemSettings.international.map.britain[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.britain[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataEU(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.europe[citynum].lat +
      "," +
      systemSettings.international.map.europe[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.europe[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataAF(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.africa[citynum].lat +
      "," +
      systemSettings.international.map.africa[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.africa[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataIN(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.india[citynum].lat +
      "," +
      systemSettings.international.map.india[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.india[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataEA(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.eastasia[citynum].lat +
      "," +
      systemSettings.international.map.eastasia[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.eastasia[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataOC(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.oceania[citynum].lat +
      "," +
      systemSettings.international.map.oceania[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.oceania[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlMapDataAU(citynum) {
    var url =
      "https://api.weather.com/v3/wx/forecast/daily/5day?geocode=" +
      systemSettings.international.map.australia[citynum].lat +
      "," +
      systemSettings.international.map.australia[citynum].lon +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var imObj = { noReport: true, locationName: "", temperature: "", icon: "" };
    $.getJSON(url, function (data) {
      ...
      imObj.dayName = data.dayOfWeek[ii];
      imObj.locationName =
        systemSettings.international.map.australia[citynum].locationName;
      imObj.temperature = data.temperatureMax[ii];
      imObj.icon = data.daypart[0].iconCodeExtend[dpi];
    }).fail(function () {
      imObj.noReport = true;
  ...
  function getIntlForecastData() {
    var url =
      "https://api.weather.com/v3/aggcommon/v3-wx-forecast-daily-7day?geocodes=";
    for (let i = 0; i < systemSettings.international.forecast.length; i++) {
      url =
        url +
        `${systemSettings.international.forecast[i].lat},${systemSettings.international.forecast[i].lon};`;
    }
    url += "&language=en-US&units=e&format=json&apiKey=" + api_key;
    ...
    $.getJSON(url, function (data) {
      data.forEach((ajaxedLoc, i) => {
        ...
        try {
          ...
          ifObj.locationName =
            systemSettings.international.forecast[i].locationName;
          for (var ji = 0; ji < ifObj.days.length; ji++, ii++, dpi = dpi + 2) {
            weatherData.internationalForecast.dayName[ji] =
              weatherData.internationalForecast.dayName[ji] == "null"
                ? ajaxedLoc["v3-wx-forecast-daily-7day"].dayOfWeek[
                    ii
                  ].substring(0, 3)
                : weatherData.internationalForecast.dayName[ji];
            if (
              weatherData.internationalForecast.dayName[ji] !=
              ajaxedLoc["v3-wx-forecast-daily-7day"].dayOfWeek[ii].substring(
                0,
                3,
              )
            ) {
            ...
            ifObj.days[ji].high =
              ajaxedLoc["v3-wx-forecast-daily-7day"].temperatureMax[ii];
            ifObj.days[ji].low =
              ajaxedLoc["v3-wx-forecast-daily-7day"].temperatureMin[ii];
            ifObj.days[ji].icon =
              ajaxedLoc["v3-wx-forecast-daily-7day"].daypart[0].iconCodeExtend[
                dpi
              ];
          }
        } catch (error) {
...
function getGolfData() {
  function getTeeTimeData() {
    var url =
      "https://api.weather.com/v3/wx/forecast/hourly/2day?" +
      locationDataHeaders.mainData.teeTime +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var currenthr = dateFns.getHours(new Date());
    $.getJSON(url, function (data) {
      ...
      switch (true) {
        ...
        case currenthr < 8:
          targetHours = [8, 10, 12, 14, 16];
          break;
        case currenthr < 10:
          targetHours = [10, 12, 14, 16, 18];
          break;
        case currenthr < 12:
          targetHours = [12, 14, 16, 18, 20];
          break;
        case currenthr < 14:
          targetHours = [14, 16, 18, 20, 6];
          break;
        case currenthr < 16:
          targetHours = [16, 18, 20, 6, 8];
          break;
        case currenthr < 18:
          targetHours = [18, 20, 6, 8, 10];
          break;
        case currenthr < 20:
          targetHours = [20, 6, 8, 10, 12];
          break;
        default:
          targetHours = [6, 8, 10, 12, 14];
      }
      ...
      for (var i = 0; i < data.validTimeLocal.length; i++) {
        if (ii < 5) {
          var dpTime = dateFns.getHours(data.validTimeLocal[i]);
          if (dpTime == targetHours[ii]) {
            weatherData.teeTime.times[ii].time = timeTitleCreate(
              data.validTimeLocal[i],
            );
            weatherData.teeTime.times[ii].icon = data.iconCodeExtend[i];
            weatherData.teeTime.times[ii].day = data.dayOfWeek[i].substring(
              0,
              3,
            );
            weatherData.teeTime.times[ii].temperature = data.temperature[i];
            weatherData.teeTime.times[ii].wind =
              data.windDirectionCardinal[i] == "CALM" || data.windSpeed[i] == 0
                ? "Calm"
                : data.windDirectionCardinal[i] + " " + data.windSpeed[i];
            ii++;
  ...
  function getCourseData(coursenum) {
    var Furl =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.courseForecast[coursenum] +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    var today = longWeekDays[new Date().getDay()];
    var golfObj = {
    ...
    var Iurl =
      "https://api.weather.com/v2/indices/golf/daypart/15day?" +
      locationDataHeaders.mainData.courseForecast[coursenum] +
      "&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(Iurl, function (data) {
      ...
      for (var i = 0; i < golfObj.days.length; i++, dpi = dpi + 2) {
        ...
        golfObj.days[i].indexName =
          data.golfIndex12hour.golfIndex[dpi] == 2
            ? "Poor"
            : data.golfIndex12hour.golfCategory[dpi];
      }
    }).fail(function () {
...
function getGardenData() {
  function getGardeningInfo() {
    var Furl =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.gardeningForecast +
      "&format=json&units=e&language=en-US&apiKey=" +
      api_key;
    $.getJSON(Furl, function (data) {
      var today = longWeekDays[new Date().getDay()];
      ...
      weatherData.gardeningIndex.locationName =
        systemSettings.garden.locationName;
      ...
      weatherData.gardeningIndex.low =
        data.dayOfWeek[ii] == today ? "none" : data.temperatureMin[ii];
      weatherData.gardeningIndex.precipChance =
        data.daypart[0].precipChance[dpi] + "%";
      weatherData.gardeningIndex.cloudCover =
        data.daypart[0].cloudCover[dpi] + "%";
    }).fail(function () {
    ...
    var Wurl =
      "https://api.weather.com/v2/indices/wateringNeeds/daypart/5day?" +
      locationDataHeaders.mainData.gardeningForecast +
      "&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(Wurl, function (data) {
      var widxWords = [
        ...
        "High",
        "High",
      ]; //0 and 1 are all the way to left, 10 is all the way to right
      var dpi = 0;
      if (data.wateringNeedsIndex12hour.dayInd[0] == "N") {
        dpi = 1;
      }
      weatherData.gardeningIndex.wateringIndex =
        data.wateringNeedsIndex12hour.wateringNeedsIndex[dpi];
      weatherData.gardeningIndex.wateringIndexWord =
        widxWords[data.wateringNeedsIndex12hour.wateringNeedsIndex[dpi]];
    }).fail(function () {
...
function getSkiData() {
  function getSkiReport(resortnum) {
    var url =
      "https://feeds.snocountry.net/getSnowReport.php?apiKey=SnoCountry.example&ids=" +
      systemSettings.ski.resorts[resortnum].resortId;
    $.getJSON(url, function (data) {
      var skiObj = {
        resortName: "",
        newSnow: "",
      ...
      skiObj.resortName =
        systemSettings.ski.resorts[resortnum].displayName +
        ", " +
        systemSettings.ski.resorts[resortnum].state;
      skiObj.newSnow =
        data.items[0].newSnowMin == "" ? '0"' : data.items[0].newSnowMin + '"';
      skiObj.baseSnowMin =
        data.items[0].avgBaseDepthMin == ""
          ? "0 - "
          : data.items[0].avgBaseDepthMin + " - ";
      skiObj.baseSnowMax =
        data.items[0].avgBaseDepthMax == ""
          ? '0"'
          : data.items[0].avgBaseDepthMax + '"';
      skiObj.surface =
        data.items[0].primarySurfaceCondition == null
          ? ""
          : data.items[0].primarySurfaceCondition;
      skiObj.openPercent =
        data.items[0].openDownHillTrails == ""
          ? "0 Trails"
          : data.items[0].openDownHillTrails == 1
            ? "1 Trail"
            : data.items[0].openDownHillTrails + " Trails";
      ...
      weatherData.skiReport.time =
        new Date().getHours() < 12
          ? new Date().toLocaleDateString("en-US", { weekday: "long" }) + " am"
          : new Date().toLocaleDateString("en-US", { weekday: "long" }) + " pm";
      var skiObj = {
        resortName:
          systemSettings.ski.resorts[resortnum].displayName +
          ", " +
  ...
  setTimeout(() => {
    weatherData.skiReport.resorts.sort(
      (a, b) => a.timeReported - b.timeReported,
    );
    //}, 500);
  }, 0);
}
function getBeachData() {
  async function getSurfReport() {
    $.get(
      "/surf/" + systemSettings.beach.surfReport.buoy + ".txt",
      function (data) {
        var stringedData = data
          .replaceAll("Peak Period", "Peak WavePreiod")
          .split(/\r?\n/);
        ...
        if (getLineNo(stringedData, "Wind:", 0) != undefined) {
          var cutWindSpeedStr = stringedData[
            getLineNo(stringedData, "Wind:", 0)
          ]
            .substring(
              stringedData[getLineNo(stringedData, "Wind:", 0)].indexOf(","),
            )
            .replaceAll(", ", "")
            .replaceAll(" kt", "");
          var cutWindDirStr =
            stringedData[getLineNo(stringedData, "Wind: ", 0)].substring(6);
          cutWindDirStr = cutWindDirStr.substring(
            0,
            cutWindDirStr.indexOf("("),
          );
          if (
            cutWindDirStr == "CALM" ||
            cutWindDirStr == "Calm" ||
            Math.round(cutWindSpeedStr * 1.151) == "0"
          ) {
        ...
        if (data.includes("Swell:") == true) {
          if (getLineNo(stringedData, "Period:", 1) != undefined) {
            var periodText = stringedData[getLineNo(stringedData, "Period:", 1)]
              .substring(8)
              .replaceAll(" sec", "");
            weatherData.surfReport.wavePeriod =
              Math.round(Number(periodText)) + " Seconds";
          } else {
            weatherData.surfReport.wavePeriod = "";
          }
        } else {
          if (getLineNo(stringedData, "Period:", 0) != undefined) {
            var periodText = stringedData[getLineNo(stringedData, "Period:", 0)]
              .substring(8)
              .replaceAll(" sec", "");
            weatherData.surfReport.wavePeriod =
              Math.round(Number(periodText)) + " Seconds";
        ...
        if (data.includes("Water Temp:")) {
          var tempText = stringedData[getLineNo(stringedData, "Water Temp:", 0)]
            .substring(12)
            .slice(0, -3)
            .replaceAll(" °F", ""); //
          weatherData.surfReport.waterTemperature = Math.round(
            Number(tempText),
          );
        } else {
        ...
        if (data.includes("Wind Wave:")) {
          var waveText = stringedData[getLineNo(stringedData, "Wind Wave:", 0)]
            .substring(11)
            .replaceAll(" ft", "");
        ...
        weatherData.surfReport.locationName =
          systemSettings.beach.surfReport.displayName != ""
            ? systemSettings.beach.surfReport.displayName
            : "DFLTNAME";
  ...
  function coastForecast() {
    $.get(
      "/coastalwaters/" + systemSettings.beach.coastalForecast.office,
      function (data) {
    ...
    if (cwfid === undefined) {
      ...
      $.get(`/cwf/${cwfid}`, function (data) {
        ...
        var stringedData = data.productText
          .replaceAll(".SUN", "X~Sunday")
          .replaceAll(".MON", "X~Monday")
          .replaceAll(".TUE", "X~Tuesday")
          .replaceAll(".WED", "X~Wednesday")
          .replaceAll(".THU", "X~Thursday")
          .replaceAll(".FRI", "X~Friday")
          .replaceAll(".SAT", "X~Saturday")
          .replaceAll(".TODAY", "X~Today")
          .replaceAll(".TONIGHT", "X~Tonight")
          .replaceAll(".THIS AFTERNOON", "X~This afternoon")
          .replaceAll(".REST OF TODAY", "X~Rest of Today")
          .replaceAll(".REST OF TONIGHT", "X~Rest of Tonight")
          .split(/\r?\n/);
        for (var i = 0; i < stringedData.length; i++) {
        ...
        var lineBump = getLineNo(
          stringedData,
          systemSettings.beach.coastalForecast.zone,
          0,
        );
        weatherData.coastalForecast.locationName =
          systemSettings.beach.coastalForecast.zoneName == ""
            ? stringedData[lineBump + 1]
            : systemSettings.beach.coastalForecast.zoneName;
        var fcstBumper = 4;
        ...
        for (var i = 0; i < stringedData.length - lineBump; i++) {
          ...
          if (!isTime(cline)) {
            ...
            if (weatherData.coastalForecast.warnings.length > 0) {
              if (
                weatherData.coastalForecast.warnings[0].substring(0, 3) == "..."
              ) {
                weatherData.coastalForecast.warnings[0] =
                  weatherData.coastalForecast.warnings[0].substring(3);
              } else {
                weatherData.coastalForecast.warnings[0] =
                  weatherData.coastalForecast.warnings[0];
              }
        ...
        for (var i = 0; i < 3; i++) {
          ...
          weatherData.coastalForecast.times[i].forecast = cline
            .replaceAll("X~", "<em>")
            .replaceAll("...", "</em><br>");
        }
      }).fail(function () {
  ...
  function getTides(station) {
    ...
    var url =
      ...
      "&end_date=" +
      dateFns.format(dateFns.addDays(new Date(), 4), "yyyy") +
      dateFns.format(dateFns.addDays(new Date(), 4), "MM") +
      dateFns.format(dateFns.addDays(new Date(), 4), "dd") +
      "&station=" +
      systemSettings.beach.tides.stations[station].id +
      "&product=predictions&datum=MLLW&time_zone=lst_ldt&interval=hilo&units=english&application=DataAPI_Sample&format=json";
    $.getJSON(url, function (data) {
      weatherData.tides.stations[station].noReport = false;
      weatherData.tides.stations[station].stationName =
        systemSettings.beach.tides.stations[station].name;
      var li = 0;
      var hi = 0;
      var index = 0;
      var tdone = false;
      while (tdone == false) {
        if (data.predictions[index].type == "L") {
          if (new Date(data.predictions[index].t) > cdate) {
            weatherData.tides.stations[station].low[li] =
              dateFns.format(new Date(data.predictions[index].t), "h:mm a") +
              " " +
              shortWeekDays[new Date(data.predictions[index].t).getDay()];
            li++;
          }
        } else if (data.predictions[index].type == "H") {
          if (new Date(data.predictions[index].t) > cdate) {
            weatherData.tides.stations[station].high[hi] =
              dateFns.format(new Date(data.predictions[index].t), "h:mm a") +
              " " +
              shortWeekDays[new Date(data.predictions[index].t).getDay()];
            hi++;
    ...
    var Surl =
      "https://api.weather.com/v3/wx/forecast/daily/5day?" +
      locationDataHeaders.mainData.tidesAlmanac +
      "&units=e&language=en-US&format=json&apiKey=" +
      api_key;
    $.getJSON(Surl, function (data) {
      weatherData.tides.almanacName =
        systemSettings.beach.tides.almanac.obsName;
      var srid = 0;
      var ssid = 0;
      ...
      weatherData.tides.sunrise =
        dateFns.format(new Date(data.sunriseTimeLocal[srid]), "h:mm a") +
        " " +
        shortWeekDays[new Date(data.sunriseTimeLocal[srid]).getDay()];
      weatherData.tides.sunset =
        dateFns.format(new Date(data.sunsetTimeLocal[ssid]), "h:mm a") +
        " " +
        shortWeekDays[new Date(data.sunsetTimeLocal[ssid]).getDay()];
    }).fail(function () {
      weatherData.tides.almanacName =
        systemSettings.beach.tides.almanac.obsName;
      weatherData.tides.sunrise = "";
...
function allData() {
  console.log("alldata");
  if (
    systemSettings.packageSettings.includes("forecast") ||
    systemSettings.packageSettings.includes("minicoreone") ||
    systemSettings.packageSettings.includes("minicoretwo")
  ) {
    getCoreData();
  }

...
function locationJS() {
  ...
  if (systemSettings.mainCity.autoFind == true) {
    ...
    if (systemSettings.upperDisplayCity.autoFind == true) {
      systemSettings.upperDisplayCity.locationName =
        systemSettings.mainCity.locationName;
      systemSettings.upperDisplayCity.obsName =
        systemSettings.mainCity.locationName;
      ...
      systemSettings.upperDisplayCity.locationID =
        systemSettings.mainCity.locationID;
    }
    ...
    if (systemSettings.golf.autoFind == true) {
      systemSettings.golf.teeTime.locationName =
        systemSettings.mainCity.locationName;
...
var mainquery =
  window.location.search == ""
    ? undefined
    : window.location.search.replaceAll("?", "").replaceAll("%20", " ");
  ...
  if (mainquery != undefined && mainquery != "nationalForecast") {
    $.getJSON(
      "https://api.weather.com/v3/location/search?query=" +
        ...
        systemSettings.apiKeys.api_key,
      function (data) {
        //maincity
        systemSettings.mainCity.locationName = data.location.displayName[0]
          .replaceAll("Charter Township", "")
          .replaceAll("Township", "");
        systemSettings.mainCity.obsName = data.location.displayName[0];
        systemSettings.mainCity.bulletinName =
          data.location.displayName[0] + " Area";
        systemSettings.mainCity.almanacLocationName =
          data.location.displayName[0];
        ...
        if (systemSettings.mainCity.radar.auto) {
          systemSettings.mainCity.radar.satCoords = [
            data.location.longitude[0],
            data.location.latitude[0],
          ];
          systemSettings.mainCity.radar.radarCities.push({
            locationName: data.location.displayName[0],
            dotTopPos: "518",
            dotLeftPos: "788",
            nameTopMargin: "-11",
            nameLeftMargin: "43",
        ...
        setMainCityBackground(
          data.location.displayName[0].toLowerCase(),
          data.location.adminDistrictCode[0],
        );
    ...
    $.getJSON(
      ...
      function (data) {
        ...
        $.getJSON(
          `https://api.weather.com/v3/location/point?geocode=${data.lat},${data.lon}&language=en-US&format=json&apiKey=${systemSettings.apiKeys.api_key}`,
          function (iddata) {
            //console.log(iddata);
            systemSettings.mainCity.locationID = iddata.location.locId;
            systemSettings.LBar.locations.cities[0].locationID =
              iddata.location.locId;
            if (systemSettings.upperDisplayCity.autoFind == true) {
              systemSettings.upperDisplayCity.locationID =
                iddata.location.locId;
            }
...
function grabNearbyCities(lat, lon) {
  ...
  if (nearbyRound >= 10) {
    ...
    console.warn(
      "Nearby pull was cut off after 10 turns, location list may not be full.",
    );
    nearbyRound = 0;
    return;
  }
  ...
  $.getJSON(
    ...
    function (data) {
      ...
      if (newCities.length >= 8) {
        ...
        grabNearbyCities(
          data.location.latitude[data.location.latitude.length - 1],
          data.location.longitude[data.location.longitude.length - 1],
        );
      }
    },
  );
...
function createNewNearbyCity(icao) {
  ...
  $.getJSON(
    ...
    for (let i = 0; i < newCities.length; i++) {
      ...
      if (cityObj.obsName == newCities[i].obsName) {
        dontPush = true;
      }
      if (cityObj.locationID == newCities[i].locationID) {
        dontPush = true;
      }
      if (newCities.length >= 8) {
        dontPush = true;
...
function grabNearbyAirports(lat, lon) {
  ...
  var airportCount = 0;
  $.getJSON(
    `https://api.weather.com/v3/location/near?geocode=${lat},${lon}&product=airport&subproduct=major&format=json&apiKey=${systemSettings.apiKeys.api_key}`,
    function (data) {
      for (let i = 0; i < data.location.iataCode.length; i++) {
        if (
          systemSettings.airport.main.some(
            (a) =>
              a.airportName === data.location.airportName[i] ||
              a.iataCode === data.location.iataCode[i],
          )
        )
          continue;
        else if (data.location.iataCode[i] == null) continue;
        else if (
          data.location.airportName[i].includes("Army") ||
          data.location.airportName[i].includes("Air Force") ||
          data.location.airportName[i].includes("Air Field") ||
          data.location.airportName[i].includes(" Base") ||
          !data.location.airportName[i].includes("Airport")
        )
          continue;
        if (airportCount < systemSettings.airport.main.length) {
          systemSettings.airport.main[airportCount].airportName =
            data.location.airportName[i];
          systemSettings.airport.main[airportCount].iataCode =
            data.location.iataCode[i];
          airportCount++;
        } else if (
          data.location.airportName[i].includes("International") ||
          data.location.airportName[i].includes("Intl")
        ) {
          systemSettings.airport.main[1].airportName =
            data.location.airportName[i];
    ...
    function (data) {
      extraCityObj = {
        ...
        radar: {
          ...
          lon: data.location.longitude,
          auto: true,
          zoom: systemSettings.mainCity.radar.zoom,
      ...
      if (data.location.displayName == systemSettings.mainCity.locationName)
        return; /*{
            if(data.location.locale["locale4"] != null){
      ...
      if (!dontPush) {
        newExtraCities.push(extraCityObj);
      }
...
function addExtraCities() {
  for (let i = 0; i < systemSettings.extraCity.maxCities; i++) {
    try {
      if (newExtraCities[i] == undefined) {
        throw new Error(
          "Extra city undefined, won't be added to extra city list.",
        );
      }
      if (
        systemSettings.extraCity.maxCities ==
  ...
  if (systemSettings.extraCity.cities.length == 0) {
    for (let i = 0; i < systemSettings.packageSettings.length; i++) {
      systemSettings.packageSettings[i] = systemSettings.packageSettings[
        i
      ].replaceAll(
        "extralocal",
        Math.random() > 0.5 ? "minicoreone" : "minicoretwo",
      );
    }
  }
}
  ...
  $.getJSON(
    ...
    function (data) {
      ...
      if (tickerArray.length < 10) {
        ...
        systemSettings.LBar.ccTicker.cities = tickerArray.sort((a, b) =>
...
function distanceByDegrees(c1, c2) {
  var lat1 = parseFloat(c1.lat),
    lon1 = parseFloat(c1.lon),
    lat2 = parseFloat(c2.lat),
    lon2 = parseFloat(c2.lon),
    dLat = lat2 - lat1,
    dLon = lon2 - lon1;
...
function initTravelMap() {
  ...
  var sysMapLeft =
    -(travelRegional[distances[0].index].leftPos - 645) > -600
      ? -600
      : -(travelRegional[distances[0].index].leftPos - 645);
  systemSettings.travel.regionalMap.mapLeft = sysMapLeft;
  var sysMapTop =
    -(travelRegional[distances[0].index].topPos - 455) > -250
      ? -250
      : -(travelRegional[distances[0].index].topPos - 455);
  systemSettings.travel.regionalMap.mapTop = sysMapTop;
  //travelLimits key: 0 = top, 1 = bottom, 2 = left, 3 = right
  var travelLimits = getTravelMapLimits(
    travelRegional[distances[0].index].type,
  );
  var travelCount = 0;
  for (let j = 0; j < distances.length; j++) {
    ...
    var travelTopLimit =
      travelRegional[distances[0].index].topPos -
      travelRegional[distances[j].index].topPos;
    var travelLeftLimit =
      travelRegional[distances[0].index].leftPos -
      travelRegional[distances[j].index].leftPos;
    if (travelTopLimit > travelLimits[0] || travelTopLimit < travelLimits[1]) {
      continue;
    }
    if (
      travelLeftLimit > travelLimits[2] ||
      travelLeftLimit < travelLimits[3]
    ) {
      continue;
    }
    systemSettings.travel.regionalMap.cities[travelCount] =
      travelRegional[distances[j].index];
    travelCount++;
  }
}
...
function initLocDataHeaders() {
  locationDataHeaders = {
    mainData: {
      ...
      outdoorActivity:
        "geocode=" +
        systemSettings.health.lat +
        "," +
        systemSettings.health.lon,
      pollen:
        "geocode/" +
        systemSettings.health.lat +
        "/" +
      ...
      teeTime:
        "geocode=" +
        systemSettings.golf.teeTime.lat +
        "," +
        systemSettings.golf.teeTime.lon,
      courseForecast: [],
      //garden
      gardeningIndex:
        "geocode=" +
        systemSettings.garden.lat +
        "," +
        systemSettings.garden.lon,
      //beach
  ...
  for (var i = 0; i < systemSettings.nearbyCities.cities.length; i++) {
    if (systemSettings.nearbyCities.cities[i].locationID.includes(":")) {
      ...
      //get from icao
      locationDataHeaders.mainData.nearbyCities[i] =
        "icaoCode=" + systemSettings.nearbyCities.cities[i].locationID;
    }
  }
  //extraloc
  for (var i = 0; i < systemSettings.extraCity.cities.length; i++) {
    try {
      if (systemSettings.extraCity.cities[i].locationID.includes(":")) {
        ...
        locationDataHeaders.mainData.alerts.extraLoc[i] =
          "geocode=" +
          systemSettings.extraCity.cities[i].lat +
          "," +
          systemSettings.extraCity.cities[i].lon;
        locationDataHeaders.mainData.currentConditions.english.extraLoc[i] =
          "icaoCode=" + systemSettings.extraCity.cities[i].locationID;
        locationDataHeaders.mainData.localForecast.extraLoc[i] =
          "geocode=" +
          systemSettings.extraCity.cities[i].lat +
          "," +
          systemSettings.extraCity.cities[i].lon;
        locationDataHeaders.mainData.dayPart.english.extraLoc[i] =
          "geocode=" +
          systemSettings.extraCity.cities[i].lat +
          "," +
          systemSettings.extraCity.cities[i].lon;
        locationDataHeaders.mainData.extendedForecast.english.extraLoc[i] =
          "geocode=" +
          systemSettings.extraCity.cities[i].lat +
          "," +
          systemSettings.extraCity.cities[i].lon;
  ...
  try {
    if (systemSettings.health.locationID.includes(":")) {
      ...
      locationDataHeaders.mainData.uvIndexCurrent =
        "icaoCode=" + systemSettings.health.locationID;
    ...
    locationDataHeaders.mainData.outdoorActivity =
      "geocode=" + systemSettings.health.lat + "," + systemSettings.health.lon;
    locationDataHeaders.mainData.pollen =
      "geocode/" + systemSettings.health.lat + "/" + systemSettings.health.lon;
    locationDataHeaders.mainData.achesBreathe =
      "geocode=" + systemSettings.health.lat + "," + systemSettings.health.lon;
  ...
  try {
    if (systemSettings.beach.tides.almanac.locationID.includes(":")) {
      ...
      locationDataHeaders.mainData.tidesAlmanac =
        "icaoCode=" + systemSettings.beach.tides.almanac.locationID;
    }
  } catch (error) {
    locationDataHeaders.mainData.tidesAlmanac =
      "icaoCode=" + systemSettings.beach.tides.almanac.locationID;
  ...
  for (var i = 0; i < systemSettings.LBar.ccTicker.cities.length; i++) {
    try {
      if (systemSettings.LBar.ccTicker.cities[i].locationID.includes(":")) {
        ...
        locationDataHeaders.ccTickerData.cities.current[i] =
          "geocode=" +
          systemSettings.LBar.ccTicker.cities[i].lat +
          "," +
          systemSettings.LBar.ccTicker.cities[i].lon;
        locationDataHeaders.ccTickerData.cities.forecast[i] =
          "geocode=" +
          systemSettings.LBar.ccTicker.cities[i].lat +
          "," +
          systemSettings.LBar.ccTicker.cities[i].lon;
  ...
  for (var i = 0; i < systemSettings.LBar.ccTicker.travelCities.length; i++) {
    try {
      if (
        systemSettings.LBar.ccTicker.travelCities[i].locationID.includes(":")
      ) {
        dataHeaderCCTTravel(i);
      } else {
        locationDataHeaders.ccTickerData.travelCities.current[i] =
          "icaoCode=" + systemSettings.LBar.ccTicker.travelCities[i].locationID;
        locationDataHeaders.ccTickerData.travelCities.forecast[i] =
          "geocode=" +
          systemSettings.LBar.ccTicker.travelCities[i].lat +
          "," +
          systemSettings.LBar.ccTicker.travelCities[i].lon;
      }
    } catch (error) {
      locationDataHeaders.ccTickerData.travelCities.current[i] =
        "icaoCode=" + systemSettings.LBar.ccTicker.travelCities[i].locationID;
  ...
  for (var i = 0; i < systemSettings.LBar.locations.cities.length; i++) {
    try {
      if (systemSettings.LBar.locations.cities[i].locationID.includes(":")) {
        ...
        locationDataHeaders.LBarData.current[i] =
          "icaoCode=" + systemSettings.LBar.locations.cities[i].locationID;
        locationDataHeaders.LBarData.forecast[i] =
          "geocode=" +
          systemSettings.LBar.locations.cities[i].lat +
          "," +
          systemSettings.LBar.locations.cities[i].lon;
      }
    } catch (error) {
      locationDataHeaders.LBarData.current[i] =
        "icaoCode=" + systemSettings.LBar.locations.cities[i].locationID;
...
function dataHeaderLBar(locNum) {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.LBar.locations.cities[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.LBar.locations.cities[locNum].lat = data.location.latitude;
      systemSettings.LBar.locations.cities[locNum].lon =
        data.location.longitude;
      locationDataHeaders.LBarData.current[locNum] =
        "geocode=" +
        systemSettings.LBar.locations.cities[locNum].lat +
        "," +
        systemSettings.LBar.locations.cities[locNum].lon;
      locationDataHeaders.LBarData.forecast[locNum] =
        "geocode=" +
        systemSettings.LBar.locations.cities[locNum].lat +
        "," +
        systemSettings.LBar.locations.cities[locNum].lon;
...
function dataHeaderCCTTravel(locNum) {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.LBar.ccTicker.travelCities[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.LBar.ccTicker.travelCities[locNum].lat =
        data.location.latitude;
      systemSettings.LBar.ccTicker.travelCities[locNum].lon =
        data.location.longitude;
      locationDataHeaders.ccTickerData.travelCities.current[locNum] =
        "geocode=" +
        systemSettings.LBar.ccTicker.travelCities[locNum].lat +
        "," +
        systemSettings.LBar.ccTicker.travelCities[locNum].lon;
      locationDataHeaders.ccTickerData.travelCities.forecast[locNum] =
        "geocode=" +
        systemSettings.LBar.ccTicker.travelCities[locNum].lat +
        "," +
        systemSettings.LBar.ccTicker.travelCities[locNum].lon;
...
function dataHeaderCCTCities(locNum) {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.LBar.ccTicker.cities[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      ...
      locationDataHeaders.ccTickerData.cities.current[locNum] =
        "geocode=" +
        systemSettings.LBar.ccTicker.cities[locNum].lat +
        "," +
        systemSettings.LBar.ccTicker.cities[locNum].lon;
      locationDataHeaders.ccTickerData.cities.forecast[locNum] =
        "geocode=" +
        systemSettings.LBar.ccTicker.cities[locNum].lat +
        "," +
        systemSettings.LBar.ccTicker.cities[locNum].lon;
...
function dataHeaderBeachAlmanac() {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.beach.tides.almanac.locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      ...
      locationDataHeaders.mainData.tidesAlmanac =
        "geocode=" +
        systemSettings.beach.tides.almanac.lat +
        "," +
        systemSettings.beach.tides.almanac.lon;
    },
  );
}
function dataHeaderGarden() {
  if (!systemSettings.garden.locationID) {
    locationDataHeaders.mainData.gardeningForecast =
      "geocode=" + systemSettings.garden.lat + "," + systemSettings.garden.lon;
    return;
  }
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.garden.locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
...
function dataHeaderCourseForecast(locNum) {
  if (!systemSettings.golf.courses[locNum].locationID) {
    locationDataHeaders.mainData.courseForecast[locNum] =
      "geocode=" +
      systemSettings.golf.courses[locNum].lat +
      "," +
      systemSettings.golf.courses[locNum].lon;
    return;
  }
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.golf.courses[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
...
function dataHeaderTeeTime() {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.golf.teeTime.locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
...
function dataHeaderIntlFcst(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.forecast[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.forecast[locNum].lat =
        data.location.latitude;
      systemSettings.international.forecast[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapAU(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.australia[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.australia[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.australia[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapOC(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.oceania[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.oceania[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.oceania[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapEA(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.eastasia[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.eastasia[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.eastasia[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapIN(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.india[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.india[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.india[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapAF(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.africa[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.africa[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.africa[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapEU(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.europe[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.europe[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.europe[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapGB(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.britain[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.britain[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.britain[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapSA(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.southamerica[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.southamerica[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.southamerica[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapCB(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.caribbean[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.caribbean[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.caribbean[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapMX(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.mexico[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.mexico[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.mexico[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderIntlMapCA(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.international.map.canada[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.international.map.canada[locNum].lat =
        data.location.latitude;
      systemSettings.international.map.canada[locNum].lon =
        data.location.longitude;
    },
  );
}
function dataHeaderHealth() {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.health.locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
...
function dataHeaderDestinationFcst(locNum) {
  ...
  $.getJSON(
    "https://api.weather.com/v3/location/point?geocode=" +
      ...
      "," +
      systemSettings.travel.destinationForecast[locNum].lon +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      systemSettings.travel.destinationForecast[locNum].lat =
...
function dataHeaderExtraCity(locNum) {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.extraCity.cities[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      ...
      locationDataHeaders.mainData.dayPart.english.extraLoc[locNum] =
        ...
        "," +
        systemSettings.extraCity.cities[locNum].lon;
      locationDataHeaders.mainData.extendedForecast.english.extraLoc[locNum] =
        "geocode=" +
        systemSettings.extraCity.cities[locNum].lat +
        "," +
        systemSettings.extraCity.cities[locNum].lon;
  ...
  $.getJSON(
    ...
    function (data) {
      ...
      locationDataHeaders.mainData.currentConditions.english.mainLoc =
        ...
        "," +
        systemSettings.mainCity.lon;
      locationDataHeaders.mainData.currentConditions.spanish.mainLoc =
        "geocode=" +
        systemSettings.mainCity.lat +
        "," +
        systemSettings.mainCity.lon;
      ...
      locationDataHeaders.mainData.dayPart.english.mainLoc =
        ...
        "," +
        systemSettings.mainCity.lon;
      locationDataHeaders.mainData.dayPart.spanish =
        "geocode=" +
        systemSettings.mainCity.lat +
        "," +
        systemSettings.mainCity.lon;
      ...
      locationDataHeaders.mainData.extendedForecast.english.mainLoc =
        ...
        "," +
        systemSettings.mainCity.lon;
      locationDataHeaders.mainData.extendedForecast.spanish =
        "geocode=" +
        systemSettings.mainCity.lat +
        "," +
        systemSettings.mainCity.lon;
...
function dataHeaderNearbyCities(locNum) {
  $.getJSON(
    "https://api.weather.com/v3/location/point?locid=" +
      systemSettings.nearbyCities.cities[locNum].locationID +
      "&language=en-US&format=json&apiKey=" +
      api_key,
    function (data) {
      ...
      locationDataHeaders.mainData.nearbyCities[locNum] =
        "geocode=" +
        systemSettings.nearbyCities.cities[locNum].lat +
        "," +
        systemSettings.nearbyCities.cities[locNum].lon;
    },
  );
}

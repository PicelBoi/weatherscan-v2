...
//health tip chooser
var tipsLib = dateFns.isWithinInterval(new Date(), {
  start: summerTipCut,
  end: winterTipCut,
})
  ? healthTipSummer
...
function switchProvider(type) {
  if (orderidx == 0) {
    if (type == "provider") {
      ...
      for (
        var i = 0;
        i < systemSettings.appearanceSettings.adImages.length;
        i++
      ) {
        ...
        case "both":
          switch (provType) {
            ...
            case "provider":
              ...
              adImageIdx++;
              if (
                adImageIdx >= systemSettings.appearanceSettings.adImages.length
              ) {
                adImageIdx = 0;
              }
              break;
        ...
        case "custom":
          for (
            ...
            if (systemSettings.appearanceSettings.adImages[i].type == "slide") {
              if (
                systemSettings.appearanceSettings.adImages[i].value ==
                slideSettings.order[orderidx].slideLineup[gidx].group
              ) {
                ...
                console.log(
                  adImageIdx,
                  systemSettings.appearanceSettings.adImages[adImageIdx]
                    .adMessage,
                );
                adImageInUse = true;
...
function slideBackground() {
  if (weatherData.severemode == true) {
    ...
    //severe
    $(".panel").css({
      background: "url(/images/assets/middle_midgrey.png)",
    ...
    $(".darkpanel").css({
      background: "url(/images/assets/middle_black.png)",
      "background-size": "1458px, 614px",
    });
    $(".brightpanel").css({
      background: "url(/images/assets/middle_reallydarkgrey.png)",
      "background-size": "1458px, 614px",
    });
    $(".dividerpanel").css({
      background: "url(/images/assets/middle_reallydarkgrey.png)",
    ...
    $(".verticaldividerpanel").css({
      background: "url(/images/assets/middle_black.png)",
      "background-size": "1458px, 614px",
    });
    $(".radar-overlay").css({
      background:
        "transparent url(/images/backgrounds/severe_map_banner_bg.png) no-repeat",
    ...
    $(".panel.dBlue").css({
      background: "url(/images/assets/middle_darkblue.png)",
      "background-size": "1458px, 614px",
    });
    $(".panel.white").css({
      background: "url(/images/assets/middle_white.png)",
    ...
    $(".dividerpanel").css({
      background: "url(/images/assets/middle_purple.png)",
      "background-size": "1458px, 614px",
    });
    $(".verticaldividerpanel").css({
      background: "url(/images/assets/middle_darkerpurple.png)",
    ...
    $(".radar-overlay").css({
      background:
        "transparent url(/images/backgrounds/map_banner_bg.png) no-repeat",
      "background-size": "100% 100%",
    });
    ...
    if (lBarData.radarUnavailable) {
      ...
      if (gidx == 0) {
        try {
          if (miniRadarAnimation != undefined) {
  ...
  setTimeout(
    ...
    slideSettings.order[orderidx].slideLineup[gidx].duration *
      (slideSettings.order[orderidx].slideLineup[gidx].group == "extralocal"
        ? systemSettings.extraCity.cities.length
        : 1) -
      4000,
  );
  //}
  ...
  if (slideSettings.order[orderidx].slideLineup[gidx].group != "intro") {
    ...
    $(".titlearrow").css(
      "border-left",
      "47px solid " +
        arrowColors[slideSettings.order[orderidx].slideLineup[gidx].group],
    );
    $("#slides-background").css({
      background:
        `transparent url(/images/backgrounds/` +
        groupBackgrounds[
          slideSettings.order[orderidx].slideLineup[gidx].group
        ] +
        `.png) no-repeat`,
  ...
  if (slideSettings.order[orderidx].slideLineup[gidx].group == "extralocal") {
    $("#slides-background").css({
      background:
        `transparent url(/images/backgrounds/` +
        systemSettings.extraCity.cities[locationid].slidesBG +
        `.png) no-repeat`,
      "background-size": "100% 100%",
    });
  }
...
var slidePrograms = {
  ...
  cityIntro() {
    ...
    $(".city-intro .background").css({
      "background-image":
        `url(/images/backgrounds/` + systemSettings.mainCity.introBG + `.png)`,
      "background-size": "100% 100%",
    });
    let d = new Date();
    let year = d.getFullYear();
    $(".city-intro .locationname").text(systemSettings.systemLocation);
    $(".city-intro .toptext-one").text(
      "Weatherscan is only available on " +
        systemSettings.appearanceSettings.providerName,
    );
    $(".city-intro .toptext-two").text(
      "© " + year + " The Weather Channel, Inc. All Rights Reserved",
    );
  ...
  upNext() {
    ...
    if (
      ...
      $(".upnext .background").css({
        "background-image":
          `url(/images/backgrounds/` +
          systemSettings.mainCity.upNextBG +
          `.png)`,
        "background-size": "100% 100%",
      });
    }
    if (slideSettings.order[orderidx].slideLineup[gidx].group == "extralocal") {
      $(".upnext .background").css({
        "background-image":
          `url(/images/backgrounds/` +
          systemSettings.extraCity.cities[locationid].upNextBG +
          `.png)`,
        "background-size": "100% 100%",
      });
    }
    ...
    for (var UNSLOT = 0, GROUPID = gidx; UNSLOT < 4; ) {
      if (GROUPID <= slideSettings.order[orderidx].slideLineup.length - 1) {
        ...
        if (
          slideSettings.order[orderidx].slideLineup[GROUPID].group ==
          "extralocal"
        ) {
          if (systemSettings.extraCity.cities.length < 2) {
            $(".upnext .upnext-slot" + divs[UNSLOT]).text(
              slideTitles[
                slideSettings.order[orderidx].slideLineup[GROUPID].group
              ],
            );
          ...
          if (remainingHeight > 534) {
            ...
            setTimeout(() => {
              $(".titlearrow").css(
                "border-left",
                "47px solid " +
                  arrowColors[slideSettings.order[0].slideLineup[gidx].group],
              );
  ...
  changeELoc() {
    ...
    if (systemSettings.extraCity.cities.length > 1) {
      ...
      if (systemSettings.LBar.locations.type == "extralocal") {
        ...
        if (lbarLocId >= systemSettings.LBar.locations.cities.length) {
          lbarLocId = 0;
        }
        loadLbarLoc("update");
      }
    ...
    if (dataTunnel.noReport == false) {
      ...
      if (locationChoice == "spanish") {
        if (dataTunnel.feelsLike == "none") {
          $(".current-conditions .labels").text(
            "Humedad\nPunto de rocio\nPresión\nViento\nRáfagas",
          );
          if (dataTunnel.pressureTrend == 0) {
            $(".current-conditions .pressure").text("E");
            $(".current-conditions .pressure").css({
              background: "url(/images/assets/blank.png)",
              "background-size": "100% 48px",
              "background-position-y": "12px",
              "background-repeat": "no-repeat",
            });
          } else if (
            dataTunnel.pressureTrend == 1 ||
            dataTunnel.pressureTrend == 3
          ) {
            $(".current-conditions .pressure").text("");
            $(".current-conditions .pressure").css({
              background: "url(/images/assets/up_arrow.png)",
              "background-size": "100% 48px",
              "background-position-y": "12px",
              "background-repeat": "no-repeat",
            });
          } else if (
            dataTunnel.pressureTrend == 2 ||
            dataTunnel.pressureTrend == 4
          ) {
            $(".current-conditions .pressure").text("");
            $(".current-conditions .pressure").css({
  ...
  nearbyCities() {
    ...
    for (var i = 0; i < 4; i++) {
      if (
        weatherData.nearbyCities.cities[i + bumper].locationName == undefined ||
        weatherData.nearbyCities.cities[i + bumper].locationName == ""
      ) {
        $(".nearby-cities .cityname" + divs[i]).text("");
        $(".nearby-cities .temperature" + divs[i]).text("");
        getIcon(
          $(".nearby-cities .icon" + divs[i]),
          "blank",
          "current",
          "small",
        );
        ...
        $(".nearby-cities .temperature" + divs[i]).text(
          weatherData.nearbyCities.cities[i + bumper].temperature,
        );
        getIcon(
          $(".nearby-cities .icon" + divs[i]),
          weatherData.nearbyCities.cities[i + bumper].icon,
          "current",
          "small",
        );
        $(".nearby-cities .wind" + divs[i]).text(
          weatherData.nearbyCities.cities[i + bumper].wind,
        );
    ...
    setTimeout(
      ...
      slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration / 2 -
        500,
    );
  },
  radarChooser() {
  ...
  dopplerRadar() {
    ...
    radarSlide = true;
    var dataTunnel =
      locationChoice == "main" || locationChoice == "spanish"
        ? systemSettings.mainCity.radar
        : systemSettings.extraCity.cities[locationid].radar;
    if (locationChoice == "spanish") {
    ...
    try {
      ...
      locradarAmenitiesTrans.jumpTo({
        center: [dataTunnel.lon, dataTunnel.lat],
      });
      ...
      locradarAmenities.setLayoutProperty(
        "i2-county-lines-conus-ak-hi-81h5x4",
        "visibility",
        "visible",
      ); //county lines
      locradarAmenities.setLayoutProperty(
        "i2-road-vectors-conus-ak-hi-4r25d3",
        "visibility",
        "visible",
      ); //black roads
      locradarAmenities.setLayoutProperty(
        "i2-road-vectors-conus-ak-hi-4r25d3 copy",
        "visibility",
        "visible",
      ); //gray roads
    } catch (error) {
  ...
  dayPart() {
    ...
    if (dataTunnel.noReport == false) {
      ...
      for (var i = 0; i < 4; i++) {
        $(".daypart-forecast .section" + divs[i] + " .time").text(
          dataTunnel.times[i].time,
        );
        getIcon(
          $(".daypart-forecast .section" + divs[i] + " .icon"),
          dataTunnel.times[i].icon,
          "forecast",
          "small",
        );
        $(".daypart-forecast .section" + divs[i] + " .condition").text(
          dataTunnel.times[i].condition,
        );
        $(".daypart-forecast .section" + divs[i] + " .temperature").text(
          dataTunnel.times[i].temperature,
        );
        $(".daypart-forecast .section" + divs[i] + " .wind").text(
          dataTunnel.times[i].wind,
        );
      }
      var temps = [];
      var barHeights = [];
      ...
      $(".daypart-forecast .section").each(function () {
        $(".daypart-forecast .section" + divs[ii] + " .bar").animate(
          { height: barHeights[ii] + "px" },
          { duration: 1000, easing: "linear" },
        );
        $(".daypart-forecast .section" + divs[ii] + " .temperature").css(
          "height",
          barHeights[ii] + "px",
        );
        ii = ii + 1;
  ...
  extendedForecast() {
    ...
    if (dataTunnel.noReport == false) {
      ...
      for (var i = 0; i < dataTunnel.days.length; i++) {
        $(".extended-forecast .column" + divs[i] + " .time").text(
          dataTunnel.days[i].day,
        );
        getIcon(
          $(".extended-forecast .column" + divs[i] + " .icon"),
          dataTunnel.days[i].icon,
          "forecast",
          "small",
        );
        $(".extended-forecast .column" + divs[i] + " .condition").text(
          dataTunnel.days[i].condition,
        );
        $(".extended-forecast .column" + divs[i] + " .high").text(
          dataTunnel.days[i].high,
        );
        $(".extended-forecast .column" + divs[i] + " .low").text(
          dataTunnel.days[i].low,
        );
      }
  ...
  almanac() {
    ...
    if (weatherData.almanac.noReport == false) {
      ...
      for (var i = 0; i < 4; i++) {
        $(".almanac .column" + divs[i] + " .phase").text(
          weatherData.almanac.moonphases[i].moon,
        );
        $(".almanac .column" + divs[i] + " .date").text(
          weatherData.almanac.moonphases[i].date,
        );
        $(".almanac .column" + divs[i] + " .icon").css({
          background:
  ...
  trafficOverview() {
    ...
    var trafZoom =
      round < 1
        ? systemSettings.traffic.zoomOut
        : systemSettings.traffic.zoomIn;
    var cap =
      slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration
        .length - 1;
  ...
  trafficReport() {
    ...
    var blockColors = {
      "LOW IMPACT": "#59cb61",
      "MEDIUM IMPACT": "#dfcd26",
      "HIGH IMPACT": "#bf2026",
    };
    if (
      weatherData.trafficReport.noReport == false &&
      weatherData.trafficReport.incidents.length > 0
    ) {
      ...
      for (var i = 0; i < indivLength; i++) {
        $(".traffic-report .incident" + divs[i]).text(
          weatherData.trafficReport.incidents[i + bump].title,
        );
        $(".traffic-report .intensity" + divs[i]).text(
          weatherData.trafficReport.incidents[i + bump].impact,
        );
        $(".traffic-report .intensitycover" + divs[i]).text(
          weatherData.trafficReport.incidents[i + bump].impact,
        );
        $(".traffic-report .intensitycover" + divs[i]).css({
          color:
            blockColors[weatherData.trafficReport.incidents[i + bump].impact],
          "background-color":
            blockColors[weatherData.trafficReport.incidents[i + bump].impact],
        });
        $(".traffic-report .description" + divs[i]).text(
          initSpace[i] + weatherData.trafficReport.incidents[i + bump].desc,
        );
      }
  ...
  trafficFlow() {
    ...
    if (
      ...
      weatherData.trafficFlow.routes.length > 0
    ) {
      if (round == 0) {
      ...
      for (var i = 0; i < 3; i++) {
        if (weatherData.trafficFlow.routes[i + bump]) {
          ...
          $(".traffic-flow .panel" + divs[i] + " .description").text(
            weatherData.trafficFlow.routes[i + bump].from +
              " to " +
              weatherData.trafficFlow.routes[i + bump].to,
          );
          $(".traffic-flow .panel" + divs[i] + " .trafficflow").text(
            weatherData.trafficFlow.routes[i + bump].speed,
          );
          $(".traffic-flow .panel" + divs[i] + " .triptime").text(
            weatherData.trafficFlow.routes[i + bump].travelTime,
          );
          $(".traffic-flow .panel" + divs[i] + " .trafficflowcover").css({
            "background-color":
              blockColors[weatherData.trafficFlow.routes[i + bump].color],
          });
          $(".traffic-flow .panel" + divs[i] + " .routeicon").css({
            "background-image":
              "url(" + weatherData.trafficFlow.routes[i + bump].routeIcon + ")",
            "background-repeat": "no-repeat",
            "background-size": "100%",
          });
          if (weatherData.trafficFlow.routes[i + bump].speed != "CLEAR") {
            ...
            $(".traffic-flow .panel" + divs[i] + " .trafficflow").css({
              "padding-top": "0px",
              "font-size": "92px",
            });
  ...
  travelForecast() {
    ...
    if (systemSettings.travel.regionalMap.enabled == true) {
      ...
      if (unAvailable == false) {
        $(".titletext").text(
          weatherData.travelForecast.dayName[round] + " Forecast",
        );
        if (round == 0) {
        ...
        for (
          var i = 0;
          i < systemSettings.travel.regionalMap.cities.length;
          i++
        ) {
          if (
            weatherData.travelForecast.cities[i].tfData[round].noReport == false
          ) {
            $(".travel-forecast .cities .city." + numToWord(i)).fadeIn(0);
            $(
              ".travel-forecast .cities .city." + numToWord(i) + " .cityname",
            ).text(
              weatherData.travelForecast.cities[i].tfData[round].locationName,
            );
            getIcon(
              $(".travel-forecast .cities .city." + numToWord(i) + " .icon"),
              weatherData.travelForecast.cities[i].tfData[round].icon,
              "forecast",
              "map",
            );
            $(
              ".travel-forecast .cities .city." +
                numToWord(i) +
                " .temperature",
            ).text(
              weatherData.travelForecast.cities[i].tfData[round].temperature,
            );
          } else {
        ...
        setTimeout(
          ...
          slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration /
            weatherData.travelForecast.dayName.length -
            500,
        );
      } else {
        fadeSlideIn($(".travel-forecast"), 0);
  ...
  destinationForecast() {
    ...
    if (unAvailable == false) {
      ...
      for (var i = 0; i < 3; i++) {
        $(".destination-forecast .daytitle" + divs[i]).text(
          weatherData.destinationForecast.dayName[i],
        );
        $(".destination-forecast .panel" + divs[i] + " .cityname").text(
          weatherData.destinationForecast.cities[i + bump].locationName,
        );
        for (var j = 0; j < 3; j++) {
          if (
            weatherData.destinationForecast.cities[i + bump].noReport == false
          ) {
            getIcon(
              $(
                ".destination-forecast .panel" +
                  divs[i] +
                  " .day" +
                  divs[j] +
                  " .icon",
              ),
              weatherData.destinationForecast.cities[i + bump].days[j].icon,
              "forecast",
              "small",
            );
            $(
              ...
              weatherData.destinationForecast.cities[i + bump].days[j].high,
            );
            $(
              ".destination-forecast .panel" +
                divs[i] +
              ...
              weatherData.destinationForecast.cities[i + bump].days[j].low,
            );
          } else {
            getIcon(
              $(
  ...
  airportConditions() {
    ...
    if (systemSettings.airport.mainEnabled == true) {
      if (weatherData.localAirportConditions[round].noReport == false) {
        ...
        $(".airport-conditions .airportname").text(
          weatherData.localAirportConditions[round].airportName,
        );
        $(".airport-conditions .arrivals .delay").html(
          weatherData.localAirportConditions[round].arrivalDelay,
        );
        $(".airport-conditions .arrivals .reason").text(
          weatherData.localAirportConditions[round].arrivalDelayReason,
        );
        $(".airport-conditions .departures .delay").html(
          weatherData.localAirportConditions[round].departureDelay,
        );
        $(".airport-conditions .departures .reason").text(
          weatherData.localAirportConditions[round].departureDelayReason,
        );
        getIcon(
          $(".airport-conditions .icon"),
          weatherData.localAirportConditions[round].icon,
          "current",
          "medium",
        );
        $(".airport-conditions .condition").text(
          weatherData.localAirportConditions[round].condition,
        );
        $(".airport-conditions .temperature").text(
          weatherData.localAirportConditions[round].temperature,
        );
  ...
  nationalAirports() {
    ...
    for (var i = 0; i < 4; i++) {
      $(".national-airports .airport" + divs[i]).text(
        weatherData.nationalAirportConditions[bump + i].airportName,
      );
      $(".national-airports .temperature" + divs[i]).text(
        weatherData.nationalAirportConditions[bump + i].temperature,
      );
      getIcon(
        $(".national-airports .icon" + divs[i]),
        weatherData.nationalAirportConditions[bump + i].icon,
        "current",
        "small",
      );
      $(".national-airports .delay" + divs[i]).html(
        weatherData.nationalAirportConditions[bump + i].delay,
      );
    }
    setTimeout(
      () => {
  ...
  internationalForecast() {
    ...
    if (unAvailable == false) {
      ...
      for (var i = 0; i < 3; i++) {
        $(".international-forecast .daytitle" + divs[i]).text(
          weatherData.internationalForecast.dayName[i],
        );
        $(".international-forecast .panel" + divs[i] + " .cityname").text(
          weatherData.internationalForecast.cities[i + bump].locationName,
        );
        for (var j = 0; j < 3; j++) {
          if (
            weatherData.internationalForecast.cities[i + bump].noReport == false
          ) {
            getIcon(
              $(
                ".international-forecast .panel" +
                  divs[i] +
                  " .day" +
                  divs[j] +
                  " .icon",
              ),
              weatherData.internationalForecast.cities[i + bump].days[j].icon,
              "forecast",
              "small",
            );
            $(
              ...
              weatherData.internationalForecast.cities[i + bump].days[j].high,
            );
            $(
              ".international-forecast .panel" +
                divs[i] +
              ...
              weatherData.internationalForecast.cities[i + bump].days[j].low,
            );
          } else {
            getIcon(
              $(
  ...
  teeTime() {
    ...
    if (weatherData.teeTime.noReport == false) {
      ...
      for (var i = 0; i < 5; i++) {
        $(".teetime .section" + divs[i] + " .time").text(
          weatherData.teeTime.times[i].time,
        );
        getIcon(
          $(".teetime .section" + divs[i] + " .icon"),
          weatherData.teeTime.times[i].icon,
          "forecast",
          "small",
        );
        $(".teetime .section" + divs[i] + " .dayname").text(
          weatherData.teeTime.times[i].day,
        );
        $(".teetime .section" + divs[i] + " .temperature").text(
          weatherData.teeTime.times[i].temperature,
        );
        $(".teetime .section" + divs[i] + " .wind").text(
          weatherData.teeTime.times[i].wind,
        );
      }
      ...
      $(".teetime .section").each(function () {
        $(".teetime .section" + divs[ii] + " .bar").animate(
          { height: barHeights[ii] + "px" },
          { duration: 1000, easing: "linear" },
        );
        $(".teetime .section" + divs[ii] + " .temperature").css(
          "height",
          barHeights[ii] + "px",
        );
        ii = ii + 1;
  ...
  courseForecast() {
    ...
    if (weatherData.courseForecast[round].noReport == false) {
      ...
      $(".course-forecast .coursename").text(
        weatherData.courseForecast[round].locationName,
      );
      for (var i = 0; i < 3; i++) {
        $(".course-forecast .section" + divs[i] + " .dayname").text(
          weatherData.courseForecast[round].days[i].day,
        );
        $(".course-forecast .section" + divs[i] + " .high").text(
          weatherData.courseForecast[round].days[i].high,
        );
        $(".course-forecast .section" + divs[i] + " .low").text(
          weatherData.courseForecast[round].days[i].low,
        );
        $(".course-forecast .section" + divs[i] + " .wind").text(
          weatherData.courseForecast[round].days[i].wind,
        );
        if (weatherData.courseForecast[round].days[i].index != "FAIL") {
          $(".course-forecast .section" + divs[i] + " .index").css({
            "margin-left":
              nameMargin[weatherData.courseForecast[round].days[i].index],
          });
        ...
        $(".course-forecast .section" + divs[i] + " .index").text(
          weatherData.courseForecast[round].days[i].indexName,
        );
        getIcon(
          $(".course-forecast .section" + divs[i] + " .icon"),
          weatherData.courseForecast[round].days[i].icon,
          "forecast",
          "medium",
        );
      }
      ...
      setTimeout(() => {
        for (var i = 0; i < 3; i++) {
          if (weatherData.courseForecast[round].days[i].index != "FAIL") {
            $(".course-forecast .section" + divs[i] + " .arrow").animate(
              {
                "margin-left":
                  weatherData.courseForecast[round].days[i].index * 29.625,
              },
              {
                duration: weatherData.courseForecast[round].days[i].index * 250,
                easing: "linear",
              },
            );
          }
      ...
      setTimeout(
        () => {
          if (round < weatherData.courseForecast.length - 1) {
            ...
            setTimeout(() => {
              ...
              currentProgram();
              for (var i = 0; i < 3; i++) {
                $(".course-forecast .section" + divs[i] + " .arrow").css({
                  "margin-left": "0px",
                });
        ...
        slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration /
          weatherData.courseForecast.length -
          500,
      );
    } else {
      $(".tempunavailable").fadeIn(500);
  ...
  outdoorActivity() {
    ...
    if (weatherData.outdoorActivity.noReport == false) {
      ...
      $(".outdoor-activity .dayname").text(
        "Forecast for " + weatherData.outdoorActivity.dayName,
      );
      getIcon(
        $(".outdoor-activity .icon"),
        weatherData.outdoorActivity.icon,
        "forecast",
        "large",
      );
      ...
      if (
        ...
        weatherData.outdoorActivity.high
      ) {
      ...
      $(".outdoor-activity .precip").text(
        weatherData.outdoorActivity.precipChance,
      );
      $(".outdoor-activity .humidity").text(
  ...
  allergyReport() {
    ...
    if (weatherData.pollen.noReport == false) {
      ...
      if (
        weatherData.pollen.pollenCountCategory == undefined ||
        ...
        $(".allergy-report .pollencategory").text(
      ...
      setTimeout(() => {
        for (var i = 0; i < 4; i++) {
          $(".allergy-report .arrow" + divs[i]).animate(
            { "margin-left": weatherData.pollen.indices[i] * 108 + "px" },
            { duration: weatherData.pollen.indices[i] * 500, easing: "linear" },
          );
        }
      }, 500);
  ...
  healthForecast() {
    ...
    if (unAvailale == false) {
      ...
      $(".health-forecast .index.two").text(
        weatherData.achesBreathe.breatheWord,
      );
      fadeSlideIn($(".health-forecast"), 500);

      setTimeout(() => {
        $(".health-forecast .arrow.one").animate(
          { "margin-left": weatherData.achesBreathe.achesIndex * 43.2 + "px" },
          {
            duration: weatherData.achesBreathe.achesIndex * 200,
            easing: "linear",
          },
        );
        $(".health-forecast .index.one").animate(
          { "margin-left": weatherData.achesBreathe.achesIndex * 43.2 + "px" },
          {
            duration: weatherData.achesBreathe.achesIndex * 200,
            easing: "linear",
        ...
        $(".health-forecast .arrow.two").animate(
          {
            "margin-left":
              flipTen(weatherData.achesBreathe.breatheIndex) * 43.2 + "px",
          },
          {
            duration: flipTen(weatherData.achesBreathe.breatheIndex) * 200,
            easing: "linear",
          },
        );
        $(".health-forecast .index.two").animate(
          {
            "margin-left":
              flipTen(weatherData.achesBreathe.breatheIndex) * 43.2 + "px",
          },
  ...
  airQuality() {
    ...
    if (weatherData.airQuality.noReport == false) {
      ...
      setTimeout(() => {
        ...
        $(".air-quality .arrow").animate(
          { "margin-bottom": weatherData.airQuality.index * 104 + "px" },
          { duration: weatherData.airQuality.index * 1000, easing: "linear" },
        );
        $(".air-quality .index").animate(
          { "margin-bottom": margins[weatherData.airQuality.index] + "px" },
          { duration: weatherData.airQuality.index * 1000, easing: "linear" },
        );
        setTimeout(() => {
  ...
  uvIndex() {
    ...
    if (unAvailable == false) {
      ...
      if (weatherData.uvIndex.current.noReport == false) {
        ...
        $(".uv-index .currentgroup .index").text(
          weatherData.uvIndex.current.index,
        );
        $(".uv-index .currentgroup .description").text(
          weatherData.uvIndex.current.word,
        );
        $(".uv-index .currentgroup .bar").animate(
          { height: Math.round(weatherData.uvIndex.current.index * m) + "px" },
          {
            duration: weatherData.uvIndex.current.index * 150,
            easing: "linear",
          },
        );
        $(".uv-index .currentgroup .index").css({
          "margin-bottom": weatherData.uvIndex.current.index * m + "px",
        });
        $(".uv-index .currentgroup .description").css({
          "margin-bottom": weatherData.uvIndex.current.index * m + "px",
        });
      ...
      if (weatherData.uvIndex.forecast.noReport == false) {
        ...
        for (var i = 0; i < 3; i++) {
          $(".uv-index .fcstgroup .time" + divs[i]).text(
            weatherData.uvIndex.forecast.times[i].hour,
          );
          $(".uv-index .fcstgroup .day" + divs[i]).text(
            weatherData.uvIndex.forecast.times[i].day,
          );
          $(".uv-index .fcstgroup .index" + divs[i]).text(
            weatherData.uvIndex.forecast.times[i].index,
          );
          $(".uv-index .fcstgroup .description" + divs[i]).text(
            weatherData.uvIndex.forecast.times[i].word,
          );
          $(".uv-index .fcstgroup .bar" + divs[i]).animate(
            {
              height:
                Math.round(weatherData.uvIndex.forecast.times[i].index * m) +
                "px",
            },
            {
              duration: weatherData.uvIndex.forecast.times[i].index * 150,
              easing: "linear",
            },
          );
          $(".uv-index .fcstgroup .index" + divs[i]).css({
            "margin-bottom":
              weatherData.uvIndex.forecast.times[i].index * m + "px",
          });
          if (weatherData.uvIndex.forecast.times[i].index == 1) {
            $(".uv-index .fcstgroup .index" + divs[i]).css({
              "padding-bottom": "10px",
            });
            $(".uv-index .fcstgroup .description" + divs[i]).css({
              "padding-bottom": "10px",
            });
  ...
  healthTip() {
    ...
    tipsLib = dateFns.isWithinInterval(new Date(), {
      start: summerTipCut,
      end: winterTipCut,
    })
      ? healthTipSummer
  ...
  gardenForecast() {
    ...
    if (weatherData.gardeningIndex.noReport == false) {
      ...
      if (weatherData.gardeningIndex.wateringIndex != "FAIL") {
        ...
        $(".garden-forecast .arrow").animate(
          {
            "margin-left":
              (weatherData.gardeningIndex.wateringIndex - 1) * 61 + "px",
          },
          {
            duration: (weatherData.gardeningIndex.wateringIndex - 1) * 250,
            easing: "linear",
          },
        );
        setTimeout(
          () => {
            ...
            $(".garden-forecast .index").css({
              display: "flex",
              "margin-left":
                nameMargin[weatherData.gardeningIndex.wateringIndex] + "px",
            });
          },
  ...
  skiReport() {
    ...
    for (var i = 0; i < Math.ceil(systemSettings.ski.resorts.length / 3); i++) {
      $(".ski-report .resortname" + divs[i]).text(
        weatherData.skiReport.resorts[i + bump].resortName,
      );
      $(".ski-report .status" + divs[i]).text(
        weatherData.skiReport.resorts[i + bump].openPercent,
      );
      $(".ski-report .newsnow" + divs[i]).text(
        weatherData.skiReport.resorts[i + bump].newSnow,
      );
      $(".ski-report .basesnow" + divs[i]).text(
        weatherData.skiReport.resorts[i + bump].baseSnowMin +
          weatherData.skiReport.resorts[i + bump].baseSnowMax,
      );
      $(".ski-report .type" + divs[i]).text(
        weatherData.skiReport.resorts[i + bump].surface,
      );
    }
    ...
    setTimeout(
      ...
      slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration /
        Math.ceil(weatherData.skiReport.resorts.length / 3) -
        500,
    );
  },
  snowfallForecast() {
  ...
  surfReport() {
    ...
    if (weatherData.surfReport.noReport == false) {
      ...
      if (weatherData.surfReport.waterTemperature != "FAIL") {
        ...
        setTimeout(() => {
          $(".surf-report .arrow").animate(
            { "margin-left": marginArr + "px" },
            { duration: marginTime, easing: "linear" },
          );
          setTimeout(() => {
            $(".surf-report .temp").text(
              weatherData.surfReport.waterTemperature,
            );
      ...
      if (weatherData.surfReport.waveHeight != "NONE") {
        ...
        if (weatherData.surfReport.waveHeight == 0) {
          ...
          if (weatherData.surfReport.waveHeight == 1) {
            $(".surf-report .feet").text("foot");
          } else {
            $(".surf-report .feet").text("feet");
          }
          $(".surf-report .wavenum").css({ "margin-bottom": bumper + "px" });
          $(".surf-report .feet").css({
            "margin-bottom": bumper + "px",
            "font-size": "40px",
            height: "59px",
          });
          setTimeout(() => {
            $(".surf-report .wavebar").animate(
              { height: 120 + bumper + "px" },
              { duration: 1000, easing: "linear" },
            );
            setTimeout(() => {
    ...
    if (weatherData.coastalForecast.noReport == false) {
      ...
      if (round == 0) {
        if (weatherData.coastalForecast.warnings.length > 0) {
          ...
          for (
            var i = 0;
            i < weatherData.coastalForecast.warnings.length;
            i++
          ) {
            alertText =
              alertText +
              weatherData.coastalForecast.warnings[i].replaceAll("...", "<br>");
          }
      ...
      $(".coastal-forecast .forecast").html(
        weatherData.coastalForecast.times[round].forecast.replaceAll(
          " NIGHT",
          " night",
        ),
      );
      ...
      setTimeout(
        ...
        slideSettings.order[orderidx].slideLineup[gidx].slides[idx].duration /
          weatherData.coastalForecast.times.length -
          rdSubtractor,
      );
    } else {
      $(".tempunavailable").fadeIn(500);
  ...
  tides() {
    ...
    var divs = [".one", ".two"];
    var unAvailable =
      weatherData.tides.stations[0].noReport == true &&
      weatherData.tides.stations[1].noReport == true
        ? true
        : false;
    if (unAvailable == false) {
      ...
      for (var i = 0; i < weatherData.tides.stations.length; i++) {
        $(".tides .locationname" + divs[i]).text(
          weatherData.tides.stations[i].stationName,
        );
        $(".tides .hightide" + divs[i]).html(
          weatherData.tides.stations[i].high[0] +
            "<br>" +
            weatherData.tides.stations[i].high[1],
        );
        $(".tides .lowtide" + divs[i]).html(
          weatherData.tides.stations[i].low[0] +
            "<br>" +
            weatherData.tides.stations[i].low[1],
        );
      }
  ...
  severeMessage() {
    ...
    if (dataTunnel.warnings.length == 0) {
      ...
      $(".headertext").text("");
      $(".severe-message .text").html(
        "Weatherscan / " +
          systemSettings.appearanceSettings.providerName +
          " brings you<br>this special programming because of<br>severe weather in your area.",
      );
...
function showSlides() {
  ...
  if (idx >= slideSettings.order[orderidx].slideLineup[gidx].slides.length) {
    ...
    try {
      if (
        systemSettings.LBar.locations.type == "extralocal" &&
        slideSettings.order[orderidx].slideLineup[gidx].group == "extralocal"
      ) {
        lbarLocId = locationid + 1;
      } else {
        if (systemSettings.LBar.locations.type != "rotate") {
          ...
          if (
            systemSettings.LBar.locations.cities.length >
              systemSettings.extraCity.cities.length + 1 &&
            slideSettings.order[orderidx].slideLineup[gidx].group !=
              "forecast" &&
            slideSettings.order[orderidx].slideLineup[gidx].group !=
              "minicoreone" &&
  ...
  currentProgram =
    slidePrograms[
      slideSettings.order[orderidx].slideLineup[gidx].slides[idx].function
    ];
...
function manageDurations() {
  switch (slideSettings.order[orderidx].slideLineup[gidx].group) {
    ...
    case "severeA":
      if (weatherData.alerts.mainLoc.pages == 0) {
        ...
        slideSettings.order[orderidx].slideLineup[gidx].slides = [
          {
            duration: 6000 * weatherData.alerts.mainLoc.pages,
            function: "bulletin",
          },
          { duration: 10000, function: "currentConditions" },
          { duration: 14000, function: "nearbyCities" },
    ...
    case "severeB":
      if (weatherData.alerts.mainLoc.pages == 0) {
        ...
        slideSettings.order[orderidx].slideLineup[gidx].slides = [
          {
            duration: 5000 * weatherData.alerts.mainLoc.pages,
            function: "bulletin",
          },
          { duration: 8000, function: "currentConditions" },
          { duration: 24000, function: "dopplerRadar" },
    ...
    case "severe2":
      if (weatherData.alerts.mainLoc.pages == 0) {
        ...
        slideSettings.order[orderidx].slideLineup[gidx].slides = [
          {
            duration: 8000 * weatherData.alerts.mainLoc.pages,
            function: "bulletin",
          },
          { duration: 52000, function: "dopplerRadar" },
        ];
    ...
    case "forecast":
      if (
        weatherData.alerts.mainLoc.pages == 0 ||
        weatherData.alerts.mainLoc.pages == undefined
      ) {
        slideSettings.order[orderidx].slideLineup[gidx].slides = [
          { duration: 10000, function: "cityIntro" },
    ...
    case "extralocal":
      ...
      for (
        var eLoc = 0;
        eLoc < systemSettings.extraCity.cities.length;
        eLoc++
      ) {
        var eSlides = [];
        if (
          weatherData.alerts.extraLoc[eLoc].pages == 0 ||
          weatherData.alerts.extraLoc[eLoc].pages == undefined
        ) {
          eSlides =
            extraSlides.noBulletin[
              Math.floor(Math.random() * extraSlides.noBulletin.length)
            ];
        } else if (weatherData.alerts.extraLoc[eLoc].pages == 1) {
          eSlides =
            extraSlides.oneBulletin[
              Math.floor(Math.random() * extraSlides.noBulletin.length)
            ];
        } else {
          eSlides =
            extraSlides.twoBulletins[
              Math.floor(Math.random() * extraSlides.noBulletin.length)
            ];
        }
  ...
  console.log(
    "current package: ",
    slideSettings.order[orderidx].slideLineup[gidx],
  );
}
function buildHeader() {
  ...
  for (let i = gidx; i < slideSettings.order[0].slideLineup.length; i++) {
    ...
    switch (slideSettings.order[0].slideLineup[i].group) {
      case "forecast":
        packageName =
          systemSettings.mainCity.scrollerDisplayName == undefined
            ? systemSettings.mainCity.locationName.toUpperCase()
            : systemSettings.mainCity.scrollerDisplayName.toUpperCase();
        break;
      case "minicoreone":
        packageName =
          systemSettings.mainCity.scrollerDisplayName == undefined
    ...
    if (packageName != "EXTRALOCAL") {
      $("#slides-scroller .package-list").append(
        `<span class=" listElement shrinkY ${i === 0 ? "main" : ""}${i === 1 ? "next" : ""}">${packageName}</span>`,
      );
    } else {
      //append all these mfs (i'll do it later)
      for (let e = 0; e < systemSettings.extraCity.cities.length; e++) {
        $("#slides-scroller .package-list").append(
          `<span class=" listElement shrinkY ${i === 0 && e === 0 ? "main" : ""}${i === 1 && e === 0 ? "next" : ""}">${systemSettings.extraCity.cities[e].locationName.toUpperCase()}</span>`,
        );
        if (e != systemSettings.extraCity.cities.length - 1)
          $("#slides-scroller .package-list").append(
            `<span class="shrinkY divider-arrow"> < </span>`,
          );
      }
    }
  ...
  for (
    let j = slideSettings.order[0].slideLineup.length;
    j < gidx + slideSettings.order[0].slideLineup.length;
    j++
  ) {}
}
function headerRefresh() {
  ...
  var sliderLeft = 0;
  var prevslides = $("#slides-scroller .package-list" + " span.main").prevAll(
    "span",
  );
  var firstWidth =
    -1 *
    ($("#slides-scroller .package-list .listElement").first().width() + 41);
  var time = ((firstWidth + 37) / -135) * 800;
  $("#slides-scroller .package-list")
    .children()
    .each((index, element) => {
  ...
  //  mgVal += 4
  //}
  $("#slides-scroller .package-list").animate(
    { "margin-left": firstWidth + "px" },
    time,
    "linear",
    function () {
      setTimeout(function () {
        ...
        var prevArrow = $(
          "#slides-scroller .package-list span:first-of-type",
        ).next("span");
        $("#slides-scroller .package-list span:first-of-type")
          .next("span")
          .next("span")

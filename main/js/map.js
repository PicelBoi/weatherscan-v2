function initTravelForecast() {
  ...
  $(".travel-forecast .basemap").css({
    "background-position-y": systemSettings.travel.regionalMap.mapTop + "px",
    "background-position-x": systemSettings.travel.regionalMap.mapLeft + "px",
    "background-size": systemSettings.travel.regionalMap.mapScale,
  });
  $(".travel-forecast .map-borders").css({
    "background-position-y": systemSettings.travel.regionalMap.mapTop + "px",
    "background-position-x": systemSettings.travel.regionalMap.mapLeft + "px",
    "background-size": systemSettings.travel.regionalMap.mapScale,
  });
  if (systemSettings.travel.regionalMap.autoFind == true) {
    $(".travel-forecast .cities").css({
      top: systemSettings.travel.regionalMap.mapTop + "px",
      left: systemSettings.travel.regionalMap.mapLeft + "px",
      position: "absolute",
    });
  }
  for (var i = 0; i < systemSettings.travel.regionalMap.cities.length; i++) {
    ...
    tfClass
      .getElementsByClassName("cities")[0]
      .getElementsByClassName("city " + numToWord(i))[0]
      .appendChild(cnChild);
    ...
    tfClass
      ...
      .getElementsByClassName("city " + numToWord(i))[0]
      .appendChild(iChild);
    $(".travel-forecast .cities .city." + numToWord(i)).css({
      top: systemSettings.travel.regionalMap.cities[i].topPos + "px",
      left: systemSettings.travel.regionalMap.cities[i].leftPos + "px",
    });
  }
}
function createRadarCities(type) {
  var header;
  if (type == "main" || type == "spanish") {
    header = systemSettings.mainCity.radar;
  } else if (type == "extra") {
    header = systemSettings.extraCity.cities[locationid].radar;
  }
  ...
  for (var i = 0; i < header.radarCities.length; i++) {
    ...
    drClass
      ...
      .getElementsByClassName("city  " + numToWord(i))[0]
      .appendChild(nChild);
    $(".doppler-radar .cities .city." + numToWord(i)).css({
      top: header.radarCities[i].dotTopPos + "px",
      left: header.radarCities[i].dotLeftPos + "px",
    });
    $(".doppler-radar .cities .city." + numToWord(i) + " .name").text(
      header.radarCities[i].locationName,
    );
    $(".doppler-radar .cities .city." + numToWord(i) + " .name").css({
      top: header.radarCities[i].nameTopMargin + "px",
      left: header.radarCities[i].nameLeftMargin + "px",
    });
    var Tchild = document.createElement("div");
    ...
    drClass
      .getElementsByClassName("cities-trans")[0]
      .getElementsByClassName("city  " + numToWord(i))[0]
      .appendChild(TdChild);
    ...
    $(".doppler-radar .cities-trans .city." + numToWord(i)).css({
      top: header.radarCities[i].dotTopPos + "px",
      left: header.radarCities[i].dotLeftPos + "px",
    });
    $(
      ".doppler-radar .cities-trans .city." + numToWord(i) + " .name-trans",
    ).text(header.radarCities[i].locationName);
    $(
      ".doppler-radar .cities-trans .city." + numToWord(i) + " .name-trans",
    ).css({
      top: header.radarCities[i].nameTopMargin + "px",
...
function createRadarIcons(type) {
  ...
  for (var i = 0; i < header.radarIcons.length; i++) {
    ...
    drClass
      ...
      .getElementsByClassName("icon  " + numToWord(i))[0]
      .appendChild(iChild);
    ...
    drClass
      .getElementsByClassName("icons-trans")[0]
      .getElementsByClassName("icon  " + numToWord(i))[0]
      .appendChild(tiChild);
    ...
    drClass
      .getElementsByClassName("icons-trans")[0]
      .getElementsByClassName("icon  " + numToWord(i))[0]
      .appendChild(tioChild);
    var titChild = document.createElement("div");
    titChild.className = "text " + header.radarIcons[i].type;
    drClass
      .getElementsByClassName("icons-trans")[0]
      .getElementsByClassName("icon  " + numToWord(i))[0]
      .appendChild(titChild);

    $(".doppler-radar .icons .icon." + numToWord(i)).css({
      top: header.radarIcons[i].topPos + "px",
      left: header.radarIcons[i].leftPos + "px",
    });
    $(".doppler-radar .icons .icon." + numToWord(i) + " .text").text(
      header.radarIcons[i].text,
    );
    $(".doppler-radar .icons-trans .icon." + numToWord(i)).css({
      top: header.radarIcons[i].topPos + "px",
      left: header.radarIcons[i].leftPos + "px",
    });
    $(".doppler-radar .icons-trans .icon." + numToWord(i) + " .text").text(
      header.radarIcons[i].text,
    );
    if (Number(header.radarIcons[i].text) >= 100) {
      $(".doppler-radar .icons .icon." + numToWord(i) + " .text").css(
        "font-size",
        "31px",
      );
      $(".doppler-radar .icons-trans .icon." + numToWord(i) + " .text").css(
        "font-size",
        "31px",
      );
    }
...
function createMiniradarCities() {
  ...
  for (var i = 0; i < systemSettings.LBar.radar.radarCities.length; i++) {
    ...
    drClass
      ...
      .getElementsByClassName("city  " + numToWord(i))[0]
      .appendChild(nChild);
    $(".radar-lbar .cities .city." + numToWord(i)).css({
      top: systemSettings.LBar.radar.radarCities[i].dotTopPos + "px",
      left: systemSettings.LBar.radar.radarCities[i].dotLeftPos + "px",
    });
    $(".radar-lbar .cities .city." + numToWord(i) + " .name").text(
      systemSettings.LBar.radar.radarCities[i].locationName,
    );
    $(".radar-lbar .cities .city." + numToWord(i) + " .name").css({
      top: systemSettings.LBar.radar.radarCities[i].nameTopMargin + "px",
      left: systemSettings.LBar.radar.radarCities[i].nameLeftMargin + "px",
    });
    var Tchild = document.createElement("div");
    Tchild.className = "city " + numToWord(i);
    drClass.getElementsByClassName("cities-trans")[0].appendChild(Tchild);
    ...
    //TnoChild.className = "name-outline shrinkY"
    //drClass.getElementsByClassName("cities-trans")[0].getElementsByClassName("city  " + numToWord(i))[0].appendChild(TnoChild)
    $(".radar-lbar .cities-trans .city." + numToWord(i)).css({
      top: systemSettings.LBar.radar.radarCities[i].dotTopPos + "px",
      left: systemSettings.LBar.radar.radarCities[i].dotLeftPos + "px",
    });
    $(".radar-lbar .cities-trans .city." + numToWord(i) + " .name-trans").text(
      systemSettings.LBar.radar.radarCities[i].locationName,
    );
    $(".radar-lbar .cities-trans .city." + numToWord(i) + " .name-trans").css({
      top: systemSettings.LBar.radar.radarCities[i].nameTopMargin + "px",
      left: systemSettings.LBar.radar.radarCities[i].nameLeftMargin + "px",
    });
...
function alignDataMaps() {
  $(".data-map").css({
    scale: systemSettings.dataMaps.zoom,
    top: systemSettings.dataMaps.topPos + "%",
    left: systemSettings.dataMaps.leftPos + "%",
  });
}
function createTrafficMapIcons(zoom) {
  ...
  var id =
    zoom == 0
      ? systemSettings.traffic.mapIcons.zoomOut
      : systemSettings.traffic.mapIcons.zoomIn;
  for (var i = 0; i < id.length; i++) {
    var child = document.createElement("div");
    child.className = "icon " + numToWord(i);
    toClass.getElementsByClassName("traffic-icons")[0].appendChild(child);

    $(".traffic-overview .traffic-icons .icon." + numToWord(i)).css({
      top: id[i].topPos + "px",
      left: id[i].leftPos + "px",
      "background-image": "url(/images/trafficMap/" + id[i].icon + ".png)",
